...
Indicate MVP is a modular monolith implemented as one TypeScript Next.js App Router application and deployed to one Vercel project. The same application serves the protected CMS, APIs, webhooks, short-lived background handlers, and one shared public news template for all configured publication hostnames.
...
10. CMS, API, Telegram, background, and reconciliation adapters invoke shared application services rather than issuing tenant SQL or reimplementing business rules.
...
|---|---:|---|---|
| Next.js App Router application | 1 | CMS, APIs, webhooks, cron handlers, public template | No tenant-specific application or build |
...
- `indicate.web.id` for CMS and authentication callbacks;
...
app/ adapters
  -> src/application/ use cases and business services
     -> src/domain/ entities, policies, state machines, value objects
        -> src/ports/ repository, auth, queue, storage, clock, ID, audit interfaces
           <- src/infrastructure/ Drizzle, Supabase Auth, R2, Upstash,
                                  Telegram, Cloudflare, Vercel adapters

src/shared/ contains transport-neutral primitives and safe cross-cutting helpers.
```
...
- CMS, API, Telegram, cron, and public adapters authenticate/resolve context, validate input, invoke one use case, and map typed outcomes.
...
### 7.1 CMS, API, and Telegram mutation flow
...
Validation, permission, tenant ownership, optimistic version, mutation, and required Audit Log precede success. Telegram owns conversational state and transport formatting only; it maps to the same service commands and schemas as CMS/API.
...
```mermaid
sequenceDiagram
    participant T as CMS API or Telegram
...
| PublicContentService | listing, detail, Category, search, feed | Published Site relation and exact context filters |
...
Domain/application code returns typed outcomes. Only adapters map them to HTTP, CMS, or Telegram presentation.
...
2. Run deterministic typecheck, lint, unit/property/integration, Playwright, dependency-policy, and secret-boundary checks.
...
7. Run smoke tests across control-plane hosts and the configured three-root/three-region matrix.
...
- lint;
- Vitest unit tests;
- Vitest/fast-check property tests with sanitized seed and minimized counterexample reporting;
- Vitest integration tests;
- non-interactive Playwright end-to-end tests;
- migration/dependency/environment/deployment checks;
- client-bundle and artifact secret scans.

Managed-service properties run against deterministic in-memory or mocked port models, with representative integration contracts retained for Supabase/PostgreSQL, R2, Upstash, Cloudflare, Vercel, and Telegram. Concurrency-sensitive invariants also receive real database uniqueness/locking cases so fakes do not hide races.
...
1. **Major Stage 1 — Foundation:** one Next.js app, approved stack, Runtime Configuration, deployment contracts, and Quality Gate tooling.
2. **Major Stage 2 — Persistence and authorization:** Drizzle schema/migrations/seed, Supabase Auth, tenant transactions, Membership, RBAC.
3. **Major Stage 3 — Business and CMS:** shared services, CMS modules, Publisher Registry, canonical Articles, filtering, Basic Analytics, Audit Logs.
4. **Major Stage 4 — Media and publication:** private R2 media, durable jobs, Upstash dispatch, idempotency, leases/fencing, bounded retries, results.
5. **Major Stage 5 — Public delivery:** Cloudflare/Vercel exact-domain activation, hostname resolver, shared public template, SEO, cache/invalidation.
6. **Major Stage 6 — External entry points:** Telegram, API Keys, rate limiting, replay defense, customer/subscription administration.
7. **Major Stage 7 — Production readiness:** automated validation across exactly three configured root domains and Wonosobo, Magelang, and Semarang.
