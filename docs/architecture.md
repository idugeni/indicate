...
Article URLs reach Meta through a separate, one-shot path. `article_sites` carries a `social_warmed_at` marker, so each public article URL is handed to the scrape endpoint once, at first publish, instead of once per mutation that touches it — an editorial edit or a publisher rename used to re-scrape every article that publisher owned. The dispatcher reads the due set oldest-first after its tasks complete, so a page is never scraped before its edge purge lands, and records a target only after Meta accepted the URL: a failed or budget-cut warm stays due and is retried on the next invocation. Withdrawing an article clears the marker, so a republication earns its single scrape again. See [ops lessons §12](ops-lessons.md#12-fb-debugger-satu-kali-per-url-saat-pertama-tayang).
