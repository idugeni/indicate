...
1. Run the complete deterministic Stage 7 gate with `npm run test:stage7`.
...
3. Reconcile the configured MVP seed and activate the apex plus Wonosobo, Magelang, and Semarang Sites for each of the exactly three runtime root hostnames.
4. Configure least-privilege production credentials through server-only environment values. Include both the Cloudflare management-plane credential used to inspect R2 privacy and the R2 S3 access-key/secret credentials used by the media data plane and read-only `HeadBucket` readiness probe. Do not place credentials in command arguments, logs, fixtures, reports, or repository files.
5. Confirm the Telegram webhook and every exact Site hostname are already configured. The readiness command is read-only.
...
The command requires `NODE_ENV=production` and `SCHEMA_GATE_MODE=live`. It validates the complete Runtime Configuration and then checks:
...
1. Preserve the Stage 7 gate diagnostics and sanitized production-readiness report as release evidence.
...
4. Re-run `npm run readiness:production` after promotion and perform bounded smoke checks against control-plane routes and configured public hosts.
...
1. Run the production-readiness command again.
...
- Telegram exposes the webhook URL but not the configured secret token. The command validates the server-side secret contract and exact URL; authenticated webhook integration tests verify the secret boundary.
...
- The command performs no write probe against production PostgreSQL, R2, Redis, Cloudflare, Vercel, or Telegram. Write behavior remains covered by deterministic and representative integration suites.
