...
1. Operate all organizations, sites, public hostnames, Dashboard workflows, APIs, jobs, and Telegram workflows from one shared application and managed-resource topology.
...
7. Reuse identical business rules across Dashboard, API, background, and Telegram entry points.
...
A User is linked uniquely to one Supabase Auth identity. A Membership associates one User, one Organization, and one Role. Roles grant named Permissions. Protected Dashboard requests require a valid Supabase Auth session, an active Membership in the selected Active Organization, and the required Permission.
...
## 8. Central Dashboard scope

The protected Dashboard is served from `indicate.web.id` and provides:
...
3. **Major Dashboard:** shared Business Services, Dashboard modules, Publisher Registry, Article lifecycle/filtering, Basic Analytics, and Audit Logs.
