...
export class CloudflareAuthorityAdapter implements CloudflareAuthorityPort {
  ...
  constructor(private readonly accountId: string, private readonly token: string, private readonly zoneIds: readonly string[], private readonly expectedNameservers: readonly string[], private readonly productionTarget: string, private readonly publicNsResolver: (hostname: string) => Promise<readonly string[]> = resolveNs, private readonly fetcher: typeof fetch = fetch) {}
  private async call<T>(path: string, init?: RequestInit): Promise<T> { const response = await this.fetcher(`https://api.cloudflare.com/client/v4${path}`, { ...init, headers: { Authorization: `Bearer ${this.token}`, 'Content-Type': 'application/json', ...init?.headers } }); if (!response.ok) throw new Error('cloudflare_unavailable'); const body = await response.json() as CloudflareEnvelope<T>; if (!body.success) throw new Error('cloudflare_unavailable'); return body.result; }
  private async zone(hostname: string, exact = false): Promise<Zone> { for (const id of this.zoneIds) { const zone = await this.call<Zone>(`/zones/${id}`); if (hostname === zone.name || (!exact && hostname.endsWith(`.${zone.name}`))) return zone; } throw new Error('cloudflare_zone_unavailable'); }
  ...
  private async inspectZone(hostname: string, exact: boolean): Promise<CloudflareZoneStatus> {
    const zone = await this.zone(hostname, exact);
  ...
  async verifyZone(hostname: string): Promise<CloudflareZoneStatus> { return this.inspectZone(hostname, false); }
  async verifyRootZone(hostname: string): Promise<CloudflareZoneStatus> { return this.inspectZone(hostname, true); }
