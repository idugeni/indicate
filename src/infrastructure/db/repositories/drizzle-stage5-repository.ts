...
import { articleSites, articles, auditLogs, authors, cacheBypasses, categories, domainActivationAttempts, domains, invalidationTasks, media, officialAffiliations, publishers, regions, sites, siteSettings } from '../schema';
import type * as schema from '../schema';

type Database = PostgresJsDatabase<typeof schema>;
type Transaction = Parameters<Parameters<Database['transaction']>[0]>[0];
const iso = (value: Date | string) => (value instanceof Date ? value : new Date(value)).toISOString();
const absoluteMediaUrl = (context: ResolvedSiteContext, mediaId: string) => `https://${context.normalizedHostname}/api/public/media/${mediaId}`;
...
export class DrizzleStage5Repository implements Stage5Repository {
  ...
  private async tenant(transaction: Transaction, actor: AuthorizedTenantActorContext): Promise<void> {
    if (!actor.permissionSet.has('sites.manage')) throw new Stage5ResourceUnavailableError();
  ...
  async findActiveSitesByExactHostname(hostname: string): Promise<readonly ResolvedSiteContext[]> {
    const rows = await this.database.execute<{
      normalized_hostname: string; organization_id: string; domain_id: string; site_id: string;
      region_id: string | null; routing_version: number; content_version: number;
    }>(sql`SELECT * FROM indicate_private.resolve_stage5_public_host(${hostname})`);
    return [...rows].map((row) => ({ normalizedHostname: row.normalized_hostname, organizationId: row.organization_id, domainId: row.domain_id, siteId: row.site_id, regionId: row.region_id, routingVersion: row.routing_version, contentVersion: row.content_version }));
  }

  async findPendingActivation(hostname: string, attemptId: string): Promise<boolean> {
    const rows = await this.database.execute<{ pending: boolean }>(sql`SELECT indicate_private.is_stage5_pending_host(${hostname}, ${attemptId}::uuid) AS pending`);
    return rows[0]?.pending === true;
  }

  async loadPublicSite(context: ResolvedSiteContext, query: PublicContentQuery): Promise<PublicSiteData | null> {
    return this.database.transaction(async (transaction) => {
      ...
      return {
        ...
        articles: rows.filter((row) => row.publishedAt !== null).map((row) => ({ id: row.id, slug: row.slug, title: row.title, description: row.body.replace(/\s+/gu, ' ').slice(0, 180), body: row.body, regionId: row.regionId, categoryId: row.categoryId, categorySlug: row.categorySlug, categoryName: row.categoryName, authorName: row.authorName, publisherName: row.publisherName, attribution: row.attribution ?? row.publisherName ?? 'Redaksi', publisherVerified: row.publisherVerification === 'verified', independent: row.publisherType === 'independent_publisher', officialInstitution: row.publisherVerification === 'verified' ? row.affiliationInstitution : null, publishedAt: iso(row.publishedAt!), updatedAt: iso(row.updatedAt), imageUrl: row.leadMediaId !== null && row.mediaState === 'active' ? absoluteMediaUrl(context, row.leadMediaId) : null, imageWidth: null, imageHeight: null })),
      };
    });
  }
  ...
  async beginActivation(actor: AuthorizedTenantActorContext, siteId: string, hostname: string, previousHostname: string | null, now: string): Promise<ActivationAttempt> {
    return this.database.transaction(async (transaction) => {
      ...
      if (existing[0] !== undefined) {
        if (existing[0].hostname !== hostname) throw new Stage5ConflictError('Another hostname activation is pending');
      ...
      const row = rows[0]; if (row === undefined || row.domain.status !== 'active' || (row.site.regionId !== null && row.region?.status !== 'active')) throw new Stage5ResourceUnavailableError();
      if (previousHostname !== null) {
        const ownership = await transaction.execute<{ owned: boolean }>(sql`
          SELECT indicate_private.is_stage5_previous_host_owned(
            ${actor.organizationId}::uuid, ${siteId}::uuid, ${previousHostname}
          ) AS owned
        `);
        if (ownership[0]?.owned !== true) throw new Stage5ConflictError('Previous hostname is not owned by Site');
      }
      const expected = row.region === null ? row.domain.normalizedHostname : `${row.region.slug}.${row.domain.normalizedHostname}`;
      if (expected !== hostname || row.site.normalizedHostname !== hostname) throw new Stage5ConflictError('Invalid exact Site hostname');
      ...
      if (duplicates.length > 0) throw new Stage5ConflictError('Hostname already mapped');
      ...
      await transaction.insert(domainActivationAttempts).values({ organizationId: actor.organizationId, id, siteId, hostname, previousHostname, operation: 'activate', phase: 'pending', status: 'pending', nextAttemptAt: date, externalStatus: {}, createdAt: date, updatedAt: date });
  ...
  private mapAttempt(row: typeof domainActivationAttempts.$inferSelect): ActivationAttempt {
    return { id: row.id, organizationId: row.organizationId, siteId: row.siteId, hostname: row.hostname, previousHostname: row.previousHostname, operation: row.operation, phase: row.phase as ActivationAttempt['phase'], status: row.status, attempts: row.attempts, nextAttemptAt: iso(row.nextAttemptAt), claimToken: row.reconciliationClaimToken, claimExpiresAt: row.reconciliationClaimExpiresAt === null ? null : iso(row.reconciliationClaimExpiresAt), externalStatus: row.externalStatus };
  }

  async updateActivation(actor: AuthorizedTenantActorContext, attemptId: string, phase: ActivationAttempt['phase'], externalStatus: Readonly<Record<string, unknown>>, now: string): Promise<ActivationAttempt> {
    return this.database.transaction(async (transaction) => {
      await this.tenant(transaction, actor);
      const rows = await transaction.update(domainActivationAttempts).set({ phase, status: phase === 'completed' ? 'completed' : 'processing', externalStatus: { ...externalStatus }, sanitizedFailure: null, updatedAt: new Date(now) }).where(and(eq(domainActivationAttempts.organizationId, actor.organizationId), eq(domainActivationAttempts.id, attemptId), inArray(domainActivationAttempts.status, ['pending', 'processing']), actor.entryPoint === 'reconciler' ? and(eq(domainActivationAttempts.reconciliationClaimToken, actor.requestId), gt(domainActivationAttempts.reconciliationClaimExpiresAt, new Date(now))) : undefined)).returning();
      if (rows[0] === undefined) throw new Stage5ResourceUnavailableError(); return this.mapAttempt(rows[0]);
    });
  }
  ...
  async failActivation(actor: AuthorizedTenantActorContext, attemptId: string, failure: Readonly<Record<string, unknown>>, nextAttemptAt: string, terminal: boolean, now: string): Promise<ActivationAttempt> {
    return this.database.transaction(async (transaction) => {
      await this.tenant(transaction, actor);
      const rows = await transaction.update(domainActivationAttempts).set({ status: terminal ? 'failed' : 'pending', phase: terminal ? 'failed' : undefined, attempts: sql`${domainActivationAttempts.attempts} + 1`, nextAttemptAt: new Date(nextAttemptAt), reconciliationClaimToken: null, reconciliationClaimExpiresAt: null, sanitizedFailure: { ...failure }, updatedAt: new Date(now) }).where(and(eq(domainActivationAttempts.organizationId, actor.organizationId), eq(domainActivationAttempts.id, attemptId), inArray(domainActivationAttempts.status, ['pending', 'processing']), actor.entryPoint === 'reconciler' ? and(eq(domainActivationAttempts.reconciliationClaimToken, actor.requestId), gt(domainActivationAttempts.reconciliationClaimExpiresAt, new Date(now))) : undefined)).returning();
      if (rows[0] === undefined) throw new Stage5ResourceUnavailableError();
  ...
  async claimActivationAttempts(now: string, limit: number, claimToken: string, claimExpiresAt: string): Promise<readonly ActivationAttempt[]> {
    const rows = await this.database.execute<typeof domainActivationAttempts.$inferSelect>(sql`
      SELECT
        ...
        previous_hostname AS "previousHostname", operation, phase, status, attempts,
      ...
      FROM indicate_private.claim_stage5_activation_attempts(
  ...
  async completeActivation(actor: AuthorizedTenantActorContext, attemptId: string, plan: InvalidationPlan, now: string): Promise<ResolvedSiteContext> {
    return this.database.transaction(async (transaction) => {
      ...
      const attempt = attempts[0]; if (attempt === undefined || attempt.phase !== 'probe_verified') throw new Stage5ConflictError();
      ...
      const site = activated[0]; if (site === undefined) throw new Stage5ResourceUnavailableError();
      await transaction.update(domainActivationAttempts).set({ phase: 'completed', status: 'completed', reconciliationClaimToken: null, reconciliationClaimExpiresAt: null, updatedAt: new Date(now) }).where(and(eq(domainActivationAttempts.organizationId, actor.organizationId), eq(domainActivationAttempts.id, attemptId)));
  ...
  async deactivateSite(actor: AuthorizedTenantActorContext, siteId: string, hostname: string, plan: InvalidationPlan, now: string): Promise<ActivationAttempt> {
    return this.database.transaction(async (transaction) => {
      ...
      if (changed.length !== 1) throw new Stage5ResourceUnavailableError();
      const id = crypto.randomUUID();
      await transaction.insert(domainActivationAttempts).values({ organizationId: actor.organizationId, id, siteId, hostname, previousHostname: hostname, operation: 'deactivate', phase: 'deactivating', status: 'pending', nextAttemptAt: new Date(now), externalStatus: {}, createdAt: new Date(now), updatedAt: new Date(now) });
      await transaction.insert(invalidationTasks).values(this.taskValues(plan, now));
  ...
  async completeDeactivation(actor: AuthorizedTenantActorContext, attemptId: string, now: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      await this.tenant(transaction, actor);
      const rows = await transaction.update(domainActivationAttempts).set({ phase: 'completed', status: 'completed', reconciliationClaimToken: null, reconciliationClaimExpiresAt: null, sanitizedFailure: null, updatedAt: new Date(now) }).where(and(eq(domainActivationAttempts.organizationId, actor.organizationId), eq(domainActivationAttempts.id, attemptId), eq(domainActivationAttempts.operation, 'deactivate'), eq(domainActivationAttempts.phase, 'deactivating'), inArray(domainActivationAttempts.status, ['pending', 'processing']), actor.entryPoint === 'reconciler' ? and(eq(domainActivationAttempts.reconciliationClaimToken, actor.requestId), gt(domainActivationAttempts.reconciliationClaimExpiresAt, new Date(now))) : undefined)).returning({ id: domainActivationAttempts.id });
      if (rows.length !== 1) throw new Stage5ResourceUnavailableError();
    });
  }

  async createInvalidation(plan: InvalidationPlan, now: string): Promise<InvalidationTask> { const value = this.taskValues(plan, now); await this.database.insert(invalidationTasks).values(value); return { ...plan, id: value.id, attempts: 0, nextAttemptAt: now, status: 'pending', claimToken: null, claimExpiresAt: null, sanitizedFailure: null }; }
  ...
  async claimInvalidations(now: string, limit: number): Promise<readonly InvalidationTask[]> {
    ...
    const rows = await this.database.execute<typeof invalidationTasks.$inferSelect>(sql`
      ...
      FROM indicate_private.claim_stage5_invalidation_tasks(
        ${now}::timestamptz, ${limit}, ${token}::uuid, ${claimExpiresAt}::timestamptz
      )
    `);
    return [...rows].map((row) => this.mapTask(row));
  }
  async completeInvalidation(task: InvalidationTask, now: string): Promise<void> { if (task.claimToken === null) throw new Stage5ConflictError('missing_claim'); const rows = await this.database.execute<{ completed: boolean }>(sql`SELECT indicate_private.complete_stage5_invalidation(${task.organizationId}::uuid, ${task.id}::uuid, ${task.claimToken}::uuid, ${now}::timestamptz) AS completed`); if (rows[0]?.completed !== true) throw new Stage5ConflictError('stale_claim'); }
  async failInvalidation(task: InvalidationTask, failure: Readonly<Record<string, unknown>>, nextAttemptAt: string, terminal: boolean, now: string): Promise<void> { if (task.claimToken === null) throw new Stage5ConflictError('missing_claim'); const rows = await this.database.execute<{ failed: boolean }>(sql`SELECT indicate_private.fail_stage5_invalidation(${task.organizationId}::uuid, ${task.id}::uuid, ${task.claimToken}::uuid, ${JSON.stringify(failure)}::jsonb, ${nextAttemptAt}::timestamptz, ${terminal}, ${now}::timestamptz) AS failed`); if (rows[0]?.failed !== true) throw new Stage5ConflictError('stale_claim'); }
}
