...
import type {
  ClaimedCleanupTask, CleanupTaskRecord, MediaAssetRecord, MediaOwner, MediaReservationRecord, PublicationJobRecord,
  PublicationStatusProjection, PublicationTargetRecord, Stage4TenantSnapshot, TargetTransitionCommit,
  TransitionReceiptRecord, WorkerClaim,
} from '@/domain/stage4/models';
import { aggregateJobState, isAllowedTargetTransition, projectPublicationResult } from '@/domain/stage4/publication-policy';
...
type ReservationRow = typeof mediaKeyReservations.$inferSelect;
type MediaRow = typeof media.$inferSelect;
type JobRow = typeof publishingJobs.$inferSelect;
type TargetRow = typeof publishingJobTargets.$inferSelect;
type ReceiptRow = typeof publicationTransitionReceipts.$inferSelect;
const iso = (value: Date) => value.toISOString();
const optionalIso = (value: Date | null) => value?.toISOString() ?? null;
const SYSTEM_REQUEST = 'stage4-reconciler';

function ownerFromRow(row: { articleId: string | null; siteId: string | null; organizationAsset: boolean }): MediaOwner {
...
function mapReservation(row: ReservationRow): MediaReservationRecord {
  return { id: row.id, organizationId: row.organizationId, objectKey: row.objectKey, purpose: row.purpose, owner: ownerFromRow(row), expectedMediaType: row.expectedMediaType, expectedSizeBytes: row.expectedSizeBytes, expectedChecksum: row.expectedChecksum, status: row.status, expiresAt: iso(row.expiresAt), createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) };
}
function mapMedia(row: MediaRow): MediaAssetRecord {
  if (row.state === 'reserved') throw new Stage4ConflictError();
  return { id: row.id, organizationId: row.organizationId, objectKey: row.objectKey, purpose: row.purpose, mediaType: row.mediaType, sizeBytes: row.sizeBytes, checksum: row.checksum, owner: ownerFromRow(row), state: row.state, version: row.version, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) };
}
function mapJob(row: JobRow): PublicationJobRecord {
...
function mapTarget(row: TargetRow & { siteId: string }): PublicationTargetRecord {
...
function mapReceipt(row: ReceiptRow): TransitionReceiptRecord {
...
function mapCleanup(row: typeof objectCleanupTasks.$inferSelect): CleanupTaskRecord {
...
export class DrizzleStage4Repository implements Stage4Repository {
  ...
  private async actorContext(transaction: Transaction, actor: AuthorizedTenantActorContext): Promise<void> { await this.context(transaction, actor.organizationId, actor.actorId, actor.requestId, actor.actorType === 'user' ? actor.verifiedAuthUserId : undefined); }
  private async authorize(transaction: Transaction, actor: AuthorizedTenantActorContext, permission: string): Promise<void> {
    if (actor.actorType !== 'user') { if (!actor.permissionSet.has(permission)) throw new Stage4AccessDeniedError(); return; }
    ...
    if (rows.length !== 1) throw new Stage4AccessDeniedError();
  }
  private async audit(
  ...
  async reserveMediaCandidate(actor: AuthorizedTenantActorContext, input: ReserveMediaCandidate): Promise<ReservationCandidateResult> {
    try {
      return await this.database.transaction(async (transaction) => {
        await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.mediaManage);
        if (input.owner.kind === 'article') {
          const rows = await transaction.select({ id: articles.id }).from(articles).where(and(eq(articles.organizationId, actor.organizationId), eq(articles.id, input.owner.articleId), inArray(articles.status, ['draft', 'active']))).limit(1);
          if (rows.length !== 1) throw new Stage4AccessDeniedError();
        } else if (input.owner.kind === 'site') {
          const rows = await transaction.select({ id: sites.id }).from(sites).where(and(eq(sites.organizationId, actor.organizationId), eq(sites.id, input.owner.siteId), eq(sites.status, 'active'))).limit(1);
          if (rows.length !== 1) throw new Stage4AccessDeniedError();
        }
        const rows = await transaction.insert(mediaKeyReservations).values({ organizationId: actor.organizationId, id: input.reservationId, objectKey: input.objectKey, purpose: input.purpose, ...ownerColumns(input.owner), expectedMediaType: input.expectedMediaType, expectedSizeBytes: input.expectedSizeBytes, expectedChecksum: input.expectedChecksum, status: 'reserved', expiresAt: new Date(input.expiresAt), createdAt: new Date(input.now), updatedAt: new Date(input.now) }).onConflictDoNothing({ target: mediaKeyReservations.objectKey }).returning();
  ...
  async markReservationOccupied(actor: AuthorizedTenantActorContext, reservationId: string, now: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.mediaManage);
      const rows = await transaction.update(mediaKeyReservations).set({ status: 'occupied', updatedAt: new Date(now) }).where(and(eq(mediaKeyReservations.organizationId, actor.organizationId), eq(mediaKeyReservations.id, reservationId), eq(mediaKeyReservations.status, 'reserved'))).returning({ id: mediaKeyReservations.id });
      if (rows.length !== 1) throw new Stage4AccessDeniedError();
    });
  }
  ...
  async activateMedia(actor: AuthorizedTenantActorContext, input: ActivateMediaInput): Promise<MediaAssetRecord> {
    return this.database.transaction(async (transaction) => {
      ...
      const reservation = reservations[0]; if (reservation === undefined) throw new Stage4AccessDeniedError();
  ...
  async rejectMedia(actor: AuthorizedTenantActorContext, reservationId: string, reason: string, now: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.mediaManage);
      const rows = await transaction.update(mediaKeyReservations).set({ status: 'occupied', updatedAt: new Date(now) }).where(and(eq(mediaKeyReservations.organizationId, actor.organizationId), eq(mediaKeyReservations.id, reservationId), eq(mediaKeyReservations.status, 'reserved'))).returning();
      const row = rows[0]; if (row === undefined) throw new Stage4AccessDeniedError();
  ...
  async archiveMedia(actor: AuthorizedTenantActorContext, mediaId: string, expectedVersion: number, now: string): Promise<MediaAssetRecord> {
    return this.database.transaction(async (transaction) => {
      ...
      const existing = existingRows[0]; if (existing === undefined || existing.state !== 'active') throw new Stage4AccessDeniedError();
      if (existing.version !== expectedVersion) throw new Stage4ConflictError();
      ...
      if (rows.length !== 1) throw new Stage4ConflictError();
  ...
  async listMedia(actor: AuthorizedTenantActorContext) {
    return this.database.transaction(async (transaction) => {
      await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.mediaRead);
  ...
  async acceptPublication(actor: AuthorizedTenantActorContext, input: AcceptPublicationInput): Promise<AcceptPublicationResult> {
    try {
      return await this.database.transaction(async (transaction) => {
        await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.publishingRequest);
        ...
        const articleRows = await transaction.select({ id: articles.id }).from(articles).where(and(eq(articles.organizationId, actor.organizationId), eq(articles.id, input.articleId), inArray(articles.status, ['draft', 'active']))).limit(1);
        if (articleRows.length !== 1) throw new Stage4AccessDeniedError();
        const distinctSites = [...new Set(input.siteIds)];
        const siteRows = await transaction.select({ id: sites.id }).from(sites).where(and(eq(sites.organizationId, actor.organizationId), inArray(sites.id, distinctSites), eq(sites.status, 'active')));
        if (siteRows.length !== distinctSites.length) throw new Stage4AccessDeniedError();
        ...
        for (let index = 0; index < distinctSites.length; index += 1) {
          ...
          if (relation === undefined) {
            relation = (await transaction.insert(articleSites).values({ organizationId: actor.organizationId, id: input.articleSiteIds[index]!, articleId: input.articleId, siteId, state: 'queued', stateOccurredAt: new Date(input.now), active: true, createdAt: new Date(input.now), updatedAt: new Date(input.now) }).returning())[0]!;
          } else if (relation.state !== 'published' && relation.state !== 'failed') {
            throw new Stage4ConflictError();
          }
          ...
          if (existingRelation[0] !== undefined) {
            const updated = await transaction.update(articleSites).set({ state: 'queued', stateOccurredAt: new Date(input.now), publishedUrl: null, publishedAt: null, sanitizedFailure: null, active: true, version: relation.version + 1, updatedAt: new Date(input.now) }).where(and(eq(articleSites.organizationId, actor.organizationId), eq(articleSites.id, relation.id), eq(articleSites.version, relation.version))).returning();
            if (updated.length !== 1) throw new Stage4ConflictError();
          }
        }
      ...
      return this.database.transaction(async (transaction) => {
        ...
        const existing = rows[0]; if (existing === undefined) throw new Stage4ConflictError();
  ...
  private async systemJobMutation(organizationId: string, jobId: string, operation: (transaction: Transaction, job: JobRow) => Promise<void>): Promise<void> {
    await this.database.transaction(async (transaction) => {
      ...
      if (rows[0] === undefined) throw new Stage4AccessDeniedError();
  ...
  async getPublication(actor: AuthorizedTenantActorContext, jobId: string) {
    return this.database.transaction(async (transaction) => {
      await this.actorContext(transaction, actor); await this.authorize(transaction, actor, STAGE4_PERMISSIONS.publishingRead);
  ...
  async claimDispatchGaps(now: string, limit: number, claimToken: string, claimExpiresAt: string) {
    const refs = await this.database.execute<{ organization_id: string; job_id: string }>(sql`SELECT * FROM indicate_private.claim_stage4_dispatch_gaps(${now}::timestamptz, ${limit}, ${claimToken}::uuid, ${claimExpiresAt}::timestamptz)`);
  ...
  async runnableTargets(claim: WorkerClaim, now: string, limit: number) {
    return this.database.transaction(async (transaction) => {
      ...
      if (jobs.length !== 1) throw new Stage4ConflictError('stale_fence');
  ...
  async transitionTarget(claim: WorkerClaim, input: TargetTransitionInput): Promise<TargetTransitionCommit> {
    return this.database.transaction(async (transaction) => {
      ...
      const job = jobs[0]; if (job === undefined) throw new Stage4ConflictError('stale_fence');
      ...
      const target = targets[0]; if (target === undefined) throw new Stage4AccessDeniedError();
      if (!isAllowedTargetTransition(target.state, input.toState)) throw new Stage4ConflictError('invalid_transition');
      if ((target.state === 'published' || target.state === 'failed') && target.state === input.toState) {
      ...
      if (updatedJobs.length !== 1) throw new Stage4ConflictError('stale_fence');
  ...
  async acknowledgeTransitionReceipt(organizationId: string, receiptId: string, now: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      ...
      if (rows.length === 0) {
        ...
        if (existing.length === 0) throw new Stage4AccessDeniedError();
      }
    });
  }
  async releaseJob(claim: WorkerClaim, now: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      ...
      const job = rows[0]; if (job === undefined) throw new Stage4ConflictError('stale_fence');
      ...
      if (updated.length !== 1) throw new Stage4ConflictError('stale_fence');
  ...
  async findExpiredLeases(now: string, limit: number) {
    const refs = await this.database.execute<{ organization_id: string; job_id: string }>(sql`SELECT * FROM indicate_private.find_stage4_expired_leases(${now}::timestamptz, ${limit})`);
    return this.loadJobs(refs);
  }
  ...
  async claimTransitionReceipts(now: string, limit: number, claimToken: string, claimExpiresAt: string): Promise<readonly TransitionReceiptRecord[]> {
    const refs = await this.database.execute<{ organization_id: string; receipt_id: string }>(sql`SELECT * FROM indicate_private.claim_stage4_transition_receipts(${now}::timestamptz, ${limit}, ${claimToken}::uuid, ${claimExpiresAt}::timestamptz)`);
  ...
  async claimCleanupTasks(now: string, limit: number, claimToken: string, claimExpiresAt: string): Promise<readonly ClaimedCleanupTask[]> {
    const refs = await this.database.execute<{ organization_id: string; task_id: string }>(sql`SELECT * FROM indicate_private.claim_stage4_cleanup_tasks(${now}::timestamptz, ${limit}, ${claimToken}::uuid, ${claimExpiresAt}::timestamptz)`);
  ...
  async snapshot(organizationId: string): Promise<Stage4TenantSnapshot | null> {
    return this.database.transaction(async (transaction) => {
      await this.context(transaction, organizationId, 'stage4-snapshot', SYSTEM_REQUEST);
      ...
      return {
        ...
        sites: siteRows.map((row) => ({ id: row.id, organizationId, active: row.status === 'active' && row.activationState === 'active', normalizedHostname: row.normalizedHostname, settingsMediaIds: settingsBySite.get(row.id) ?? [] })),
        articleSites: relationRows.map((row) => ({ id: row.id, organizationId, articleId: row.articleId, siteId: row.siteId, active: row.active, state: row.state, publishedUrl: row.publishedUrl, publishedAt: optionalIso(row.publishedAt), version: row.version })),
        reservations: reservationRows.map(mapReservation), media: mediaRows.filter((row) => row.state !== 'reserved').map(mapMedia), cleanupTasks: cleanupRows.map(mapCleanup),
        invalidationIntents: invalidationRows.map((row) => ({ id: row.id, organizationId, siteId: row.siteId, reason: row.reason, tags: row.tags, status: row.status })),
        jobs: jobRows.map(mapJob), targets: targetRows.map(({ target, siteId }) => mapTarget({ ...target, siteId })), transitionReceipts: receiptRows.map(mapReceipt),
