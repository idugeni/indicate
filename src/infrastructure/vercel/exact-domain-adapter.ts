...
export class VercelExactDomainAdapter implements VercelHostingPort {
  ...
  constructor(private readonly projectId: string, private readonly teamId: string, private readonly token: string) {}
  private async call(path: string, init?: RequestInit) { const response = await fetch(`https://api.vercel.com${path}${path.includes('?') ? '&' : '?'}teamId=${encodeURIComponent(this.teamId)}`, { ...init, headers: { Authorization: `Bearer ${this.token}`, 'Content-Type': 'application/json', ...init?.headers } }); if (!response.ok) throw new Error('vercel_unavailable'); return response.json() as Promise<Record<string, unknown>>; }
  ...
  async associateExactDomain(hostname: string): Promise<ExactDomainAssociationResult> { this.requireExact(hostname); try { return this.result(hostname, await this.call(`/v10/projects/${this.projectId}/domains`, { method: 'POST', body: JSON.stringify({ name: hostname }) })); } catch { return this.result(hostname, await this.call(`/v9/projects/${this.projectId}/domains/${encodeURIComponent(hostname)}`)); } }
