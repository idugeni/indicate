...
export class DrizzlePublishingRepository implements PublishingRepository {
  ...
  async unpublishTargets(actor: AuthorizedTenantActorContext, input: PublicationTargetSelection): Promise<PublicationStatusProjection> {
    return this.database.transaction(async (transaction) => {
      ...
      for (const { target, siteId, articleId } of published) {
        ...
        await transaction.update(articleSites).set({ state: 'unpublished', stateOccurredAt: now, publishedUrl: null, publishedAt: null, socialWarmedAt: null, version: sql`${articleSites.version} + 1`, updatedAt: now }).where(and(eq(articleSites.organizationId, actor.organizationId), eq(articleSites.id, target.articleSiteId)));
  ...
  async transitionTarget(claim: WorkerClaim, input: TargetTransitionInput): Promise<TargetTransitionCommit> {
    return this.database.transaction(async (transaction) => {
      ...
      await transaction.update(articleSites).set({ state: input.toState, stateOccurredAt: new Date(input.now), publishedUrl: input.toState === 'published' ? input.publishedUrl ?? null : null, publishedAt: input.toState === 'published' ? new Date(input.now) : null, socialWarmedAt: input.toState === 'unpublished' ? null : undefined, sanitizedFailure: input.sanitizedError ?? null, attempt: input.toState === 'processing' ? target.attempt + 1 : target.attempt, ...(seededViews === null ? {} : { viewCount: seededViews }), version: sql`${articleSites.version} + 1`, updatedAt: new Date(input.now) }).where(and(eq(articleSites.organizationId, claim.organizationId), eq(articleSites.id, target.articleSiteId)));
