...
export class DrizzleDashboardRepository implements DashboardRepository {
  ...
  private async load(transaction: Transaction, organizationId: string): Promise<DashboardTenantState> {
    ...
    return {
      ...
      regions: regionRows.map((row) => ({ id: row.id, organizationId, externalKey: row.externalKey, name: row.name, slug: row.slug, status: row.status, kind: row.kind, parentRegionId: row.parentRegionId, version: row.version, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) })),
  ...
  private async persist(transaction: Transaction, actorId: string, before: DashboardTenantState, state: MutableTenantState, pendingAudits: readonly AuditRecord[]): Promise<void> {
    ...
    for (const row of state.regions) await transaction.insert(regions).values({ organizationId: state.organizationId, id: row.id, externalKey: row.externalKey, name: row.name, slug: row.slug, status: row.status, kind: row.kind, parentRegionId: row.parentRegionId, version: row.version, createdAt: new Date(row.createdAt), updatedAt: new Date(row.updatedAt) }).onConflictDoUpdate({ target: [regions.organizationId, regions.id], set: { externalKey: row.externalKey, name: row.name, slug: row.slug, status: row.status, kind: row.kind, parentRegionId: row.parentRegionId, version: row.version, updatedAt: new Date(row.updatedAt) } });
