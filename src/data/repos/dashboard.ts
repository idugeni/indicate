...
export class DrizzleDashboardRepository implements DashboardRepository {
  ...
  private async load(transaction: Transaction, organizationId: string): Promise<DashboardTenantState> {
    ...
    return {
      ...
      domains: domainRows.map((row) => ({ id: row.id, organizationId, normalizedHostname: row.normalizedHostname, status: row.status, cloudflareZoneId: row.cloudflareZoneId, routingVersion: row.routingVersion, version: row.version, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) })),
      ...
      articles: articleRows.map((row) => ({ id: row.id, organizationId, regionId: row.regionId, publisherId: row.publisherId, categoryId: row.categoryId, authorId: row.authorId, slug: row.slug, title: row.title, dek: row.dek, excerpt: row.excerpt, canonicalUrl: row.canonicalUrl, body: row.body, bodyJson: (row.bodyJson ?? null) as unknown | null, source: row.source, tags: [...row.tags], status: row.status, publishedAt: optionalIso(row.publishedAt), scheduledAt: optionalIso(row.scheduledAt), archivedAt: optionalIso(row.archivedAt), version: row.version, createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) })),
  ...
  private async persist(transaction: Transaction, actorId: string, before: DashboardTenantState, state: MutableTenantState, pendingAudits: readonly AuditRecord[]): Promise<void> {
    ...
    for (const row of state.articles) await transaction.insert(articles).values({ organizationId: state.organizationId, id: row.id, regionId: row.regionId, publisherId: row.publisherId, categoryId: row.categoryId, authorId: row.authorId, slug: row.slug, title: row.title, dek: row.dek, excerpt: row.excerpt, canonicalUrl: row.canonicalUrl, body: row.body, bodyJson: (row.bodyJson ?? null) as Record<string, unknown> | null, source: row.source, tags: [...row.tags], status: row.status, publishedAt: row.publishedAt === null ? null : new Date(row.publishedAt), scheduledAt: row.scheduledAt === null ? null : new Date(row.scheduledAt), archivedAt: row.archivedAt === null ? null : new Date(row.archivedAt), version: row.version, createdAt: new Date(row.createdAt), updatedAt: new Date(row.updatedAt) }).onConflictDoUpdate({ target: [articles.organizationId, articles.id], set: { regionId: row.regionId, publisherId: row.publisherId, categoryId: row.categoryId, authorId: row.authorId, slug: row.slug, title: row.title, dek: row.dek, excerpt: row.excerpt, canonicalUrl: row.canonicalUrl, body: row.body, bodyJson: (row.bodyJson ?? null) as Record<string, unknown> | null, source: row.source, tags: [...row.tags], status: row.status, scheduledAt: row.scheduledAt === null ? null : new Date(row.scheduledAt), archivedAt: row.archivedAt === null ? null : new Date(row.archivedAt), version: row.version, updatedAt: new Date(row.updatedAt) } });
  ...
  private async recordArticleRevisions(transaction: Transaction, actorId: string, before: DashboardTenantState, state: MutableTenantState): Promise<void> {
    for (const row of state.articles) {
      ...
      if (prior !== undefined && prior.title === row.title && prior.dek === row.dek && prior.body === row.body && JSON.stringify(prior.bodyJson ?? null) === JSON.stringify(row.bodyJson ?? null)) continue;
      ...
      await transaction.insert(articleRevisions).values({ organizationId: state.organizationId, id: crypto.randomUUID(), articleId: row.id, revisionNumber, title: row.title, dek: row.dek, body: row.body, bodyJson: (row.bodyJson ?? null) as Record<string, unknown> | null, snapshot: { slug: row.slug, excerpt: row.excerpt, source: row.source, tags: [...row.tags], status: row.status, version: row.version }, createdBy: actorId, createdAt: new Date(row.updatedAt), updatedAt: new Date(row.updatedAt) });
    }
  }
