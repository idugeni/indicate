...
import { publicApiKeyRecord, type CustomerProjection, type StoredApiKey, type SubscriptionRecord, type TelegramConversation, type TelegramIdentity, type TelegramMappingRecord, type WebhookReplayClaim } from '@/modules/integrations/models';
...
type RawApiKeyRow = {
  ...
  expires_at: RawTimestamp | null; last_used_at: RawTimestamp | null; version: number; region_id: string | null; created_at: RawTimestamp; updated_at: RawTimestamp;
};
...
function mapRawClaim(row: RawClaimRow): WebhookReplayClaim {
  ...
  type CustomerRow = { id: string; name: string; slug: string; status: 'active' | 'inactive' | 'archived'; customer_metadata: Record<string, unknown>; version: number; created_at: RawTimestamp; updated_at: RawTimestamp; subscription_status: SubscriptionRecord['status'] | null; subscription_version: number | null; subscription_created_at: RawTimestamp | null; subscription_updated_at: RawTimestamp | null };
function mapCustomer(row: CustomerRow): CustomerProjection {
  return { customer: { id: row.id, name: row.name, slug: row.slug, status: row.status, customerMetadata: row.customer_metadata, version: row.version, createdAt: normalizeIntegrationsTimestamp(row.created_at), updatedAt: normalizeIntegrationsTimestamp(row.updated_at) }, subscription: row.subscription_status === null || row.subscription_version === null || row.subscription_created_at === null || row.subscription_updated_at === null ? null : { organizationId: row.id, status: row.subscription_status, version: row.subscription_version, createdAt: normalizeIntegrationsTimestamp(row.subscription_created_at), updatedAt: normalizeIntegrationsTimestamp(row.subscription_updated_at) } };
}
...
export class DrizzleIntegrationsRepository implements IntegrationsRepository {
  ...
  private async actorContext(tx: Transaction, actor: AuthorizedTenantActorContext): Promise<void> { await this.context(tx, actor.organizationId, actor.actorId, actor.requestId, actor.actorType === 'user' ? actor.verifiedAuthUserId : undefined); await tx.execute(sql`SELECT indicate_private.set_region_context(${actor.regionScopeId ?? null}::uuid)`); }
  ...
  private keyValues(input: NewStoredApiKey) { return { organizationId: input.organizationId, id: input.id, lookupId: input.lookupId, name: input.name, salt: input.salt, verificationHash: input.verificationHash, scopes: [...input.scopes], status: 'active' as const, predecessorId: input.predecessorId, expiresAt: input.expiresAt === null ? null : new Date(input.expiresAt), regionId: input.regionId, version: 1, createdAt: new Date(input.now), updatedAt: new Date(input.now) }; }

  private async requireOwnRegion(tx: Transaction, actor: AuthorizedTenantActorContext, regionId: string | null): Promise<void> {
    if (regionId === null) return;
    const rows = await tx.select({ id: regions.id }).from(regions).where(and(eq(regions.organizationId, actor.organizationId), eq(regions.id, regionId))).limit(1);
    if (rows.length !== 1) throw new IntegrationsAccessDeniedError();
  }
  ...
  async createCustomer(actor: AuthorizedTenantActorContext, input: { readonly organizationId: string; readonly name: string; readonly slug: string; readonly customerMetadata: Readonly<Record<string, unknown>>; readonly subscription?: { readonly status: SubscriptionRecord['status'] }; readonly now: string }) {
    try {
      return await this.database.transaction(async (tx) => {
        ...
        await tx.execute(sql`SELECT indicate_private.customer_create(${actor.actorId}::uuid, ${actor.requestId}, ${input.organizationId}::uuid, ${input.name}, ${input.slug}, ${JSON.stringify(input.customerMetadata)}::jsonb, ${input.subscription === undefined ? 'active' : input.subscription.status}, ${input.now}::timestamptz)`);
