...
import { IntegrationsAccessDeniedError, IntegrationsConflictError, IntegrationsQuotaExceededError, IntegrationsSubscriptionInactiveError, type NewStoredApiKey, type ReplayClaimInput, type ReplayClaimResult, type IntegrationsRepository } from '@/modules/integrations/ports';
...
function mapTelegramMapping(row: typeof telegramIdentityMappings.$inferSelect): TelegramMappingRecord {
  return { id: row.id, organizationId: row.organizationId, userId: row.userId, roleId: row.roleId, telegramUserId: row.telegramUserId, telegramChatId: row.telegramChatId, status: row.status, version: row.version, createdAt: normalizeIntegrationsTimestamp(row.createdAt), updatedAt: normalizeIntegrationsTimestamp(row.updatedAt) };
}
type RawClaimRow = {
