...
--
-- Reviewed sources, in journal order (181 migrations):
...
-- ----------------------------------------------------------------------
-- 20260924010000_telegram_removal
-- ----------------------------------------------------------------------
...
-- Body digest (reproducible): LF-normalize this file, substitute the 64-hex
-- checksum literal below with 64 zeros, SHA-256 the complete UTF-8 bytes.
DROP TRIGGER IF EXISTS membership_telegram_mapping_guard ON public.memberships;
DROP TRIGGER IF EXISTS telegram_mapping_membership_role_guard ON public.telegram_identity_mappings;
DROP FUNCTION IF EXISTS indicate_private.enforce_membership_telegram_coherence();
DROP FUNCTION IF EXISTS indicate_private.enforce_telegram_membership_role();
DROP FUNCTION IF EXISTS indicate_private.resolve_telegram_identity(text, text);
DROP FUNCTION IF EXISTS indicate_private.list_telegram_identities(text, text);
DROP FUNCTION IF EXISTS indicate_private.outbox_enqueue(uuid, text, text, timestamptz);
DROP FUNCTION IF EXISTS indicate_private.outbox_claim(timestamptz, integer);
DROP FUNCTION IF EXISTS indicate_private.outbox_ack(uuid, boolean, integer, text, timestamptz);
DROP FUNCTION IF EXISTS indicate_private.outbox_broadcast_targets(uuid);
DROP FUNCTION IF EXISTS indicate_private.outbox_list_platform(uuid);
DROP FUNCTION IF EXISTS indicate_private.outbox_list(uuid, uuid);
DROP TABLE IF EXISTS public.telegram_conversations;
DROP TABLE IF EXISTS public.telegram_outbox;
DROP TABLE IF EXISTS public.telegram_identity_mappings;
DROP TYPE IF EXISTS public.telegram_conversation_step;
DELETE FROM public.role_permissions WHERE permission_id IN (SELECT id FROM public.permissions WHERE name = 'telegram.manage');
DELETE FROM public.permissions WHERE name = 'telegram.manage';
CREATE OR REPLACE FUNCTION indicate_private.actor_has_tenant_permission(p_permission text)
RETURNS boolean
LANGUAGE sql
STABLE
...
-- ----------------------------------------------------------------------
-- 20260925120000_jateng_network_all_domains
-- ----------------------------------------------------------------------
...
INSERT INTO drizzle."__drizzle_migrations" ("hash", "created_at") VALUES ('85675c4e7feb124d30c77744fbf896b4eb6d5482fe445264189611700be5abdc', 1790376000000);
COMMIT;
