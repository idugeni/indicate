...
import { buildStructuredObjectKey } from '@/domain/stage4/object-key';
import type { MediaAssetRecord } from '@/domain/stage4/models';
import type { IdentifierGenerator } from '@/ports/identifier-generator';
import type { ExactObjectAuthorization, ObjectStoragePort } from '@/ports/object-storage';
import { Stage4AccessDeniedError, Stage4ConflictError, type Stage4Repository } from '@/ports/stage4-repository';
...
import { mediaArchiveSchema, mediaCompletionSchema, mediaReadSchema, mediaReservationSchema } from './schemas';
...
export class MediaService {
  ...
  async reserveUpload(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<UploadReservationResult, PublicErrorEnvelope>> {
    ...
    try {
      ...
      if (error instanceof Stage4AccessDeniedError) return this.denied(actor, 'media.upload.reserve.denied', 'media');
      return this.failure(actor);
    }
  }
  ...
  async completeUpload(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<MediaAssetRecord, PublicErrorEnvelope>> {
    ...
    try {
      ...
      if (error instanceof Stage4AccessDeniedError) return this.denied(actor, 'media.upload.complete.denied', 'media');
      return this.failure(actor);
    }
  }
  ...
  async list(actor: AuthorizedTenantActorContext): Promise<Result<readonly MediaAssetRecord[], PublicErrorEnvelope>> {
    ...
    catch (error) { return error instanceof Stage4AccessDeniedError ? this.denied(actor, 'media.list.denied', 'media') : this.failure(actor); }
  }
  ...
  async authorizeTenantRead(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<ExactObjectAuthorization, PublicErrorEnvelope>> {
    ...
    } catch (error) { return error instanceof Stage4AccessDeniedError ? this.denied(actor, 'media.access.denied', 'media') : this.failure(actor); }
  }
  ...
  async archive(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<MediaAssetRecord, PublicErrorEnvelope>> {
    ...
    try { return { ok: true, value: await this.repository.archiveMedia(actor, parsed.data.mediaId, parsed.data.expectedVersion, this.clock.now().toISOString()) }; }
      ...
      if (error instanceof Stage4AccessDeniedError) return this.denied(actor, 'media.archive.denied', 'media');
      if (error instanceof Stage4ConflictError) return { ok: false, error: createPublicError('CONFLICT', 'The media record was changed by another operation.', actor.requestId) };
      return this.failure(actor);
    }
  }
}
