...
import type { ApiKeyRecord, IssuedApiKey } from '@/domain/stage6/models';
import { STAGE6_PERMISSIONS } from '@/domain/stage6/permissions';
import type { IdentifierGenerator } from '@/ports/identifier-generator';
import { Stage6AccessDeniedError, Stage6ConflictError, type NewStoredApiKey, type Stage6Repository } from '@/ports/stage6-repository';
import { createNonDisclosingDenial, createPublicError, type PublicErrorEnvelope } from '@/shared/errors/application-error';
import type { Result } from '@/shared/types/result';
import { apiKeyIssueSchema, apiKeyRevokeSchema, apiKeyRotateSchema } from './schemas';
...
export class ApiKeyService {
  ...
  private canManage(actor: AuthorizedTenantActorContext): boolean { return actor.permissionSet.has(STAGE6_PERMISSIONS.apiKeyManage); }
  private scopesAllowed(actor: AuthorizedTenantActorContext, scopes: readonly string[]): boolean {
    return scopes.every((scope) => actor.permissionSet.has(scope) && scope !== STAGE6_PERMISSIONS.customerAdmin);
  }
  ...
  async issue(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<IssuedApiKey, PublicErrorEnvelope>> {
    ...
    try {
      ...
      if (error instanceof Stage6AccessDeniedError) return this.denied(actor, 'api_key.issue.denied');
      if (error instanceof Stage6ConflictError) return { ok: false, error: createPublicError('CONFLICT', 'The API key could not be issued.', actor.requestId) };
  ...
  async rotate(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<IssuedApiKey, PublicErrorEnvelope>> {
    ...
    try {
      ...
      if (error instanceof Stage6AccessDeniedError) return this.denied(actor, 'api_key.rotate.denied');
      if (error instanceof Stage6ConflictError) return { ok: false, error: createPublicError('CONFLICT', 'The API key changed before rotation.', actor.requestId) };
  ...
  async revoke(actor: AuthorizedTenantActorContext, raw: unknown): Promise<Result<ApiKeyRecord, PublicErrorEnvelope>> {
    ...
    catch (error) { return error instanceof Stage6AccessDeniedError ? this.denied(actor, 'api_key.revoke.denied') : error instanceof Stage6ConflictError ? { ok: false, error: createPublicError('CONFLICT', 'The API key changed before revocation.', actor.requestId) } : { ok: false, error: createPublicError('DEPENDENCY_UNAVAILABLE', 'The API key could not be revoked.', actor.requestId) }; }
  }

  async list(actor: AuthorizedTenantActorContext): Promise<Result<readonly ApiKeyRecord[], PublicErrorEnvelope>> {
    if (!actor.permissionSet.has(STAGE6_PERMISSIONS.apiKeyRead) && !this.canManage(actor)) return this.denied(actor, 'api_key.list.denied');
    try { return { ok: true, value: await this.repository.listApiKeys(actor) }; } catch (error) { return error instanceof Stage6AccessDeniedError ? this.denied(actor, 'api_key.list.denied') : { ok: false, error: createPublicError('DEPENDENCY_UNAVAILABLE', 'API keys are temporarily unavailable.', actor.requestId) }; }
  }
}