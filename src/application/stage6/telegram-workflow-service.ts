...
import type { TenantBusinessService } from '@/application/stage3/tenant-business-service';
import type { MediaService } from '@/application/stage4/media-service';
import type { PublicationService } from '@/application/stage4/publication-service';
import type { AuthorizedTenantActorContext } from '@/domain/context/operation-context';
import type { TelegramConversation, TelegramIdentity, TelegramUpdate, TelegramWorkflowResult, WebhookReplayClaim } from '@/domain/stage6/models';
import type { Stage6Repository } from '@/ports/stage6-repository';
...
export class TelegramWorkflowService {
  constructor(
    private readonly repository: Stage6Repository,
  ...
  private async reply(_chatId: string, text: string): Promise<TelegramWorkflowResult> { return { reply: text }; }
  ...
  private async execute(identity: TelegramIdentity, actor: AuthorizedTenantActorContext, shared: TelegramSharedServices, update: TelegramUpdate): Promise<Result<TelegramWorkflowResult, PublicErrorEnvelope>> {
    ...
    if (text === '/cancel') { await this.repository.clearTelegramConversation(identity); return { ok: true, value: await this.reply(update.chatId, 'Conversation cancelled.') }; }
    if (text === '/regions') {
      ...
      return { ok: true, value: await this.reply(update.chatId, values.length === 0 ? 'No active Regions.' : values.join('\n')) };
    }
    if (text === '/sites') return { ok: true, value: await this.reply(update.chatId, 'Usage: /sites ARTICLE_ID to list active Sites in the Article Region.') };
    if (text === '/article') { await this.repository.saveTelegramConversation(identity, this.conversation(identity, 'article_region', {})); return { ok: true, value: await this.reply(update.chatId, 'Send the active Region ID.') }; }
    if (text.startsWith('/image ')) {
      const [, articleId] = words(text); if (articleId === undefined) return { ok: true, value: await this.reply(update.chatId, 'Usage: /image ARTICLE_ID') };
      await this.repository.saveTelegramConversation(identity, this.conversation(identity, 'article_image', { articleId })); return { ok: true, value: await this.reply(update.chatId, 'Send one image document.') };
    }
    if (text.startsWith('/sites ')) {
      const [, articleId] = words(text); if (articleId === undefined) return { ok: true, value: await this.reply(update.chatId, 'Usage: /sites ARTICLE_ID') };
      const listed = await shared.articles.listEditorial(actor); if (!listed.ok) return listed;
      ...
      if (available.length === 0) return { ok: true, value: await this.reply(update.chatId, 'No active Sites are available for this Article Region.') };
      ...
      return { ok: true, value: await this.reply(update.chatId, `Send comma-separated active Site IDs for ${article.regionId}:\n${available.map(({ id, normalizedHostname }) => `${normalizedHostname}: ${id}`).join('\n')}`) };
    }
    if (text.startsWith('/publish ')) {
      ...
      return { ok: true, value: { ...(await this.reply(update.chatId, `Publication ${publication.value.job.state}. Job: ${publication.value.job.id}`)), businessResult: publication.value } };
    }
    if (text.startsWith('/status ') || text.startsWith('/links ')) {
      ...
      return { ok: true, value: { ...(await this.reply(update.chatId, reply)), businessResult: status.value } };
    }
    const conversation = await this.repository.readTelegramConversation(identity);
    if (conversation === null || new Date(conversation.expiresAt) <= this.clock.now()) return { ok: true, value: await this.reply(update.chatId, 'Commands: /article, /image ARTICLE_ID, /regions, /sites ARTICLE_ID, /publish ARTICLE_ID SITE_IDS KEY, /status JOB_ID, /links JOB_ID.') };
  ...
  private async advance(identity: TelegramIdentity, actor: AuthorizedTenantActorContext, shared: TelegramSharedServices, update: TelegramUpdate, conversation: TelegramConversation): Promise<Result<TelegramWorkflowResult, PublicErrorEnvelope>> {
    const text = update.text?.trim() ?? '';
    const next = async (step: TelegramConversation['step'], field: string, value: unknown, reply: string) => { await this.repository.saveTelegramConversation(identity, this.conversation(identity, step, { ...conversation.data, [field]: value })); return { ok: true as const, value: await this.reply(update.chatId, reply) }; };
    ...
    if (conversation.step === 'article_slug') {
      ...
      return { ok: true, value: { ...(await this.reply(update.chatId, `Article created: ${created.value.id}`)), businessResult: created.value } };
    }
    if (conversation.step === 'article_sites') {
      ...
      await this.repository.clearTelegramConversation(identity); return { ok: true, value: { ...(await this.reply(update.chatId, `Assigned ${assigned.value.length} Site(s).`)), businessResult: assigned.value } };
    }
    if (conversation.step === 'article_image') {
      if (update.document === null) return { ok: true, value: await this.reply(update.chatId, 'Send a valid document for this Article.') };
      ...
      await this.repository.clearTelegramConversation(identity); return { ok: true, value: { ...(await this.reply(update.chatId, `Image uploaded: ${completed.value.id}`)), businessResult: completed.value } };
    }
    await this.repository.clearTelegramConversation(identity); return { ok: true, value: await this.reply(update.chatId, 'Conversation reset. Start again with /article.') };
  }
}
