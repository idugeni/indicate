...
const rawRuntimeConfigSchema = z
  .object({
    ...
    STAGE2_E2E_MODE: z.literal('1').optional(),
    CMS_HOST: hostnameSchema.default('indicate.web.id'),
  ...
  .superRefine((value, context) => {
    ...
    if (value.STAGE2_E2E_MODE === '1' && (value.NODE_ENV !== 'test' || value.APP_ENVIRONMENT !== 'test')) {
      context.addIssue({ code: 'custom', path: ['STAGE2_E2E_MODE'], message: 'e2e_mode_test_only' });
    }
    const controlHosts = [value.CMS_HOST, value.API_HOST, value.WEBHOOK_HOST];
    if (new Set(controlHosts).size !== controlHosts.length) {
      context.addIssue({ code: 'custom', path: ['CMS_HOST'], message: 'control_hosts_must_be_distinct' });
    }
...
function toRuntimeConfig(value: ParsedRuntimeEnvironment): RuntimeConfig {
  ...
  return Object.freeze({
    ...
    hosts: Object.freeze({
      ...
      reserved: new Set([value.CMS_HOST, value.API_HOST, value.WEBHOOK_HOST]),
    }),
    supabase: Object.freeze({
      projectRef: value.SUPABASE_PROJECT_REF,
      url: value.NEXT_PUBLIC_SUPABASE_URL,
      anonKey: value.NEXT_PUBLIC_SUPABASE_ANON_KEY,
