...
async function handlePOST(request: Request) {
  ...
  const limited = await limiter.enforce(limiter.publicKey('resend-webhook', sourceIdentity), { ...config.rateLimits.webhook, failureMode: 'closed' }, requestId);
  ...
  const service = production.emailWebhooks;
  if (service === null) return NextResponse.json(createNonDisclosingDenial(requestId), { status: 503 });
  const rawBody = await request.text();
  const result = await service.process(rawBody, {
    'svix-id': request.headers.get('svix-id'), 'svix-timestamp': request.headers.get('svix-timestamp'), 'svix-signature': request.headers.get('svix-signature'),
  }, requestId);
