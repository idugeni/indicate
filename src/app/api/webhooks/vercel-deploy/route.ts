 ...
 *
 * @param error - Envelope produced by `VercelDeployWebhookService`.
...
async function handlePOST(request: Request) {
  ...
  const limited = await limiter.enforce(limiter.publicKey('vercel-deploy-webhook', sourceIdentity), { ...config.rateLimits.webhook, failureMode: 'closed' }, requestId);
  ...
  const deployWebhooks = production.deployWebhooks;
  if (deployWebhooks === null) return NextResponse.json(createNonDisclosingDenial(requestId), { status: 503 });
  const rawBody = await request.text();
  const result = await deployWebhooks.process(rawBody, request.headers.get('x-vercel-signature'), requestId);
  return result.ok ? NextResponse.json({ data: result.value, requestId }) : NextResponse.json(result.error, { status: status(result.error) });
}

export const POST = withApiAccess('POST /api/webhooks/vercel-deploy', handlePOST);
