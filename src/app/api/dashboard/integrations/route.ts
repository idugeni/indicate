...
async function contextFor(organizationId: string, requestId: string): Promise<ContextResult> {
  ...
  const context = await getServerRuntimeContext(); const config = context.legacy; const runtime = createRuntimeDatabase(config); const authorization = new DrizzleAuthorizationRepository(runtime.db); const local = await resolveVerifiedLocalUser(identity, authorization, new UuidGenerator()); if (!local.ok) { await runtime.close(); return createNonDisclosingDenial(requestId); }
