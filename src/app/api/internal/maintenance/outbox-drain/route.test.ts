...
import { authorized } from '@/app/api/internal/maintenance/outbox-drain/route';

function requestWith(auth: string | null): Request {
  const headers = new Headers();
  if (auth !== null) headers.set('authorization', auth);
  return new Request('https://indicate.web.id/api/internal/maintenance/outbox-drain', { headers });
}

describe('authorized outbox-drain', () => {
