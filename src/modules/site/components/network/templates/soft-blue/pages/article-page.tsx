...
import { articleImage, formatDate, formatFullViews, isLocalImageSrc, readingMinutes } from '@/modules/site/components/network/templates/soft-blue/lib/format';
      ...
      <SoftBlueContainer className="py-8 md:py-12">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60 sm:px-5">
            <p className="m-0 flex min-w-0 items-center gap-3">
              <span aria-hidden="true" className="flex h-11 w-11 flex-none items-center justify-center rounded-full bg-[#2563eb]/10 font-sans text-base font-bold text-[#2563eb]">
                {bylineInitial}
              </span>
          ...
          <p className="m-0 mt-8 flex items-center gap-1.5 font-sans text-xs text-slate-600">
            ...
            <span>
              ...
              <Link href={`/report?artikel=${encodeURIComponent(article.slug)}`} className="font-semibold text-[#2563eb] hover:underline">
                Laporkan konten
              </Link>
            </span>
          </p>
