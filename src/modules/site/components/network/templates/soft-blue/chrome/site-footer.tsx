...
import { SOCIAL_ORDER } from '@/modules/site/company-contact';
import { channelIcon } from '@/modules/site/components/network/channel-icons';
import { getSiteCategoryNav } from '@/modules/site/components/network/templates/soft-blue/server/site-nav';
import { SoftBlueStoreBadges } from '@/modules/site/components/network/templates/soft-blue/chrome/store-badges';
          ...
          <ul className="m-0 mt-4 list-none space-y-2.5 p-0">
            {categories.map((item) => (
              <li key={`${item.href}:${item.label}`} className="m-0 p-0">
                <Link href={item.href} className="font-sans text-sm text-slate-600 transition-colors hover:text-[#2563eb]">
                  {item.label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-slate-200">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-2 px-4 py-4 text-center font-sans text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:text-left">
          ...
          <p className="m-0 flex items-center gap-3">
            <Link href="/kebijakan-privasi" className="transition-colors hover:text-[#2563eb]">
              Privasi
            </Link>
            <span aria-hidden="true" className="text-slate-300">|</span>
            <Link href="/syarat-ketentuan" className="transition-colors hover:text-[#2563eb]">
              Syarat
            </Link>
            <span aria-hidden="true" className="text-slate-300">|</span>
            <Link href="/kontak" className="transition-colors hover:text-[#2563eb]">
              Kontak
            </Link>
          </p>
        </div>
