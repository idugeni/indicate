...
export function SoftBlueTicker({ articles }: { readonly articles: readonly ArticleListItem[] }) {
  ...
  return (
    <div
      className="relative flex flex-col items-stretch gap-2.5 rounded-2xl bg-[#e6efff] p-3 shadow-sm ring-1 ring-[#c9dcff]/70 sm:flex-row sm:items-center sm:gap-3 sm:rounded-full sm:py-2 sm:pl-2 sm:pr-3"
    ...
    >
      ...
      <p
        ...
        aria-live="polite"
      >
        <Link href={`/${article.slug}`} className="no-underline hover:text-[var(--tpl-primary,#2563eb)]">
            ...
            {items.map((item, position) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  position === index ? 'w-5 bg-[var(--tpl-primary,#2563eb)]' : 'w-1.5 bg-slate-300 hover:bg-slate-400'
                }`}
              />
            ))}
          </span>
        ...
        {items.length > 1 ? (
          <span className="flex flex-none items-center gap-1.5">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full ring-1 ring-slate-200 transition-colors hover:text-[var(--tpl-primary,#2563eb)]"
            >
