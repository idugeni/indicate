...
export function SoftBlueShareButtons({ article, canonical }: { readonly article: ArticleListItem; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[var(--tpl-primary,#2563eb)]';
  const channel =
  ...
  return (
    <p className="m-0 flex w-full flex-wrap items-center justify-center gap-2 sm:w-auto sm:justify-start" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--tpl-primary,#2563eb)] text-white transition-colors hover:bg-[var(--tpl-primary-dark,#1d4ed8)]"
      >
