 ...
 */
export function SoftBlueShareButtons({ article, canonical }: { readonly article: ArticleListItem; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#2563eb]';
  ...
  return (
    <p className="m-0 flex flex-none flex-wrap items-center justify-center gap-2 sm:justify-start" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[#2563eb] text-white transition-colors hover:bg-[#1d4ed8]"
      >
      ...
      <button
        ...
        className={`${round} ${copied ? '!bg-[#2563eb] !text-white !ring-[#2563eb]' : ''}`}
      >
        {copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Link2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <span aria-live="polite" className="font-sans text-xs font-semibold text-[#2563eb]">
        {copied ? 'Tersalin!' : ''}
      </span>
    </p>
  );
