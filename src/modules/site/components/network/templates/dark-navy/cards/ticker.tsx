...
export function DarkNavyTicker({ articles }: { readonly articles: readonly NetworkArticle[] }) {
  ...
  return (
    <div
      className="flex flex-col items-stretch gap-2.5 rounded-2xl bg-[#0e1a33] p-3 shadow-sm ring-1 ring-[#1b2c4f] sm:flex-row sm:items-center sm:gap-3 sm:rounded-full sm:py-2 sm:pl-2 sm:pr-3"
    ...
    >
      ...
      <p
        key={article.id}
        className={`m-0 min-w-0 flex-1 font-sans text-[15px] font-medium leading-snug text-[#eaf0fb] line-clamp-2 sm:text-sm sm:line-clamp-1 ${reduceMotion ? '' : 'ticker-enter'}`}
        aria-live="polite"
      >
      ...
      <time dateTime={article.updatedAt} className="flex-none font-sans text-xs tabular-nums text-[#9aa9c4] sm:block">
            ...
            {items.map((item, position) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  position === index ? 'w-5 bg-[#2f7bff]' : 'w-1.5 bg-[#1b2c4f] hover:bg-[#5f6f8c]'
                }`}
              />
            ))}
          </span>
        ...
        {items.length > 1 ? (
          <span className="flex flex-none items-center gap-1.5">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full ring-1 ring-[#1b2c4f] transition-colors hover:text-[#2f7bff]"
            >
              <ChevronLeft className="h-4 w-4 text-[#9aa9c4]" aria-hidden="true" />
            </button>
            <button
              type="button"
              onClick={() => go(index + 1)}
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full ring-1 ring-[#1b2c4f] transition-colors hover:text-[#2f7bff]"
            >
              <ChevronRight className="h-4 w-4 text-[#9aa9c4]" aria-hidden="true" />
            </button>
          </span>
        ) : null}
      </div>
