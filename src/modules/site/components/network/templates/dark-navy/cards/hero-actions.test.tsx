...
import { DarkNavyHeroActions } from '@/modules/site/components/network/templates/dark-navy/cards/hero-actions';
...
describe('DarkNavyHeroActions', () => {
  it('menyimpan dan melepas simpanan lokal', () => {
    render(<DarkNavyHeroActions slug="berita-x" title="Judul X" />);
    ...
    expect(window.localStorage.getItem('dark-navy:bookmark:berita-x')).toBe('1');
  ...
  it('memulihkan status tersimpan dari localStorage', () => {
    window.localStorage.setItem('dark-navy:bookmark:berita-x', '1');
    render(<DarkNavyHeroActions slug="berita-x" title="Judul X" />);
