...
export function DarkNavyShareButtons({ article, canonical }: { readonly article: ArticleListItem; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-[var(--tpl-muted,#9aa9c4)] ring-1 ring-[var(--tpl-ring,#1b2c4f)] transition-colors hover:text-[var(--tpl-primary,#2f7bff)]';
  const channel =
    'flex h-9 w-9 items-center justify-center rounded-full ring-1 ring-[var(--tpl-ring,#1b2c4f)] transition-colors hover:text-white hover:ring-transparent';
