...
import { articleImage, isLocalImageSrc, readingMinutes } from '@/modules/site/components/network/templates/dark-navy/lib/format';
...
export function DarkNavyPickCard({ article, index }: { readonly article: NetworkArticle; readonly index: number }) {
  const src = articleImage(article);
  const reading = readingMinutes(article);
  const badge = badgeStyle(index);
  const publisherName = article.attribution;
  return (
    <Card className="flex h-full flex-col overflow-hidden rounded-2xl border-0 bg-[#0e1a33] text-[#eaf0fb] shadow-sm ring-1 ring-[#1b2c4f]/60">
      ...
      <CardHeader className="flex-1 px-5 pt-4">
        ...
        <CardDescription className="line-clamp-3 font-sans text-sm leading-relaxed text-[#9aa9c4]">
          {article.description}
        </CardDescription>
      </CardHeader>
