...
describe('WarmEditorialViewBeacon', () => {
  it('mengirim beacon sekali saat mount', () => {
    const send = installBeacon(true);
    const { container } = render(<WarmEditorialViewBeacon organizationId={O} siteId={S} articleSiteId={A} />);
  ...
  it('memakai fetch saat beacon ditolak', async () => {
    ...
    try {
      render(<WarmEditorialViewBeacon organizationId={O} siteId={S} articleSiteId={A} />);
  ...
  it('diam untuk id tidak valid', () => {
    const send = installBeacon(true);
    render(<WarmEditorialViewBeacon organizationId="bukan-uuid" siteId={S} articleSiteId={A} />);
