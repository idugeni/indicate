...
export function WarmEditorialNewsletter() {
  return (
    <section id="newsletter" aria-label="Berlangganan newsletter" className="relative overflow-hidden rounded-2xl bg-[#fae7d7] px-6 py-8 md:px-10" style={{ scrollMarginTop: '5rem' }}>
        ...
        <div>
          <p className="m-0 flex flex-col gap-2.5 sm:flex-row">
            ...
            <input
              ...
              className="h-11 w-full flex-1 appearance-none rounded-full border border-slate-200 bg-white px-4 font-sans text-base text-slate-900 placeholder:text-slate-400 focus:border-[#b4532a] focus:outline-none sm:text-sm"
            />
            {/* Pendaftaran dinonaktifkan sengaja hingga backend newsletter tersedia. */}
            <Link
              href=""
              className="inline-flex h-11 flex-none items-center justify-center rounded-full bg-[#b4532a] px-6 font-sans text-sm font-bold text-white transition-colors hover:bg-[#8a3c1d]"
            >
              Berlangganan
            </Link>
          </p>
      ...
      <Send aria-hidden="true" className="pointer-events-none absolute -right-4 bottom-6 hidden h-28 w-28 rotate-12 text-[#b4532a]/25 md:block" />
    </section>
  );
}
