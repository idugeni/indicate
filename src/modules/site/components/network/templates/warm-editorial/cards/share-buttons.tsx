 ...
 */
export function WarmEditorialShareButtons({ article, canonical }: { readonly article: NetworkArticle; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#b4532a]';
  ...
  return (
    <p className="m-0 flex flex-none flex-wrap items-center gap-2" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[#b4532a] text-white transition-colors hover:bg-[#8a3c1d]"
      >
      ...
      <button
        ...
        className={`${round} ${copied ? '!bg-[#b4532a] !text-white !ring-[#b4532a]' : ''}`}
      >
        {copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Link2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <span aria-live="polite" className="font-sans text-xs font-semibold text-[#b4532a]">
        {copied ? 'Tersalin!' : ''}
      </span>
    </p>
  );
