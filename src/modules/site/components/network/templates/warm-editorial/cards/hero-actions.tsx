...
export function WarmEditorialHeroActions({ slug, title }: { readonly slug: string; readonly title: string }) {
  const [saved, setSaved] = useState(() => {
    try {
      return typeof window !== 'undefined' && window.localStorage.getItem(`warm-editorial:bookmark:${slug}`) === '1';
    } catch {
      return false;
    }
  });
  ...
  const toggleSave = () => {
    const next = !saved;
    try {
      window.localStorage.setItem(`warm-editorial:bookmark:${slug}`, next ? '1' : '0');
    } catch {
  ...
  const button =
    'flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#b4532a] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#b4532a]';
  ...
  return (
    <p className="m-0 flex flex-none items-center gap-2">
      <button
        ...
        className={`${button} ${saved ? '!bg-[#b4532a] !text-white !ring-[#b4532a]' : ''}`}
      >
      ...
      <button
        ...
        className={`${button} ${copied ? '!bg-[#b4532a] !text-white !ring-[#b4532a]' : ''}`}
      >
