  ...
  return (
    <div className="space-y-6">
      <Link
        ...
        className={`block rounded-xl px-4 py-3 font-sans text-[15px] font-semibold ${path === '/' ? 'bg-[#fae7d7] text-[#b4532a]' : 'text-slate-800 hover:bg-slate-100'}`}
      >
        Beranda
      </Link>
      <Collapsible>
          ...
          {categories.map((item) => (
            <li key={`${item.href}:${item.label}`} className="m-0 p-0">
              <Link
                href={item.href}
                aria-current={path === item.href ? 'page' : undefined}
                className={`block rounded-xl px-4 py-2.5 font-sans text-sm font-medium ${path === item.href ? 'bg-[#fae7d7] text-[#b4532a]' : 'text-slate-700 hover:bg-slate-100'}`}
              >
                {item.label}
              </Link>
            </li>
          ...
          {INFO_LINKS.map((item) => (
            <li key={item.href} className="m-0 p-0">
              <Link
                href={item.href}
                aria-current={path === item.href ? 'page' : undefined}
                className={`block rounded-xl px-4 py-2.5 font-sans text-sm ${path === item.href ? 'bg-[#fae7d7] font-semibold text-[#b4532a]' : 'text-slate-600 hover:bg-slate-100'}`}
              >
                {item.label}
              </Link>
            </li>
