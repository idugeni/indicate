...
describe('WarmEditorialDesktopNav', () => {
  it('menampilkan beranda, empat kategori, dan pemicu lainnya', () => {
    render(<WarmEditorialDesktopNav categories={kanal(7)} path="/" />);
  ...
  it('menandai path aktif', () => {
    render(<WarmEditorialDesktopNav categories={kanal(3)} path="/kat-2" />);
...
describe('WarmEditorialMobileNav', () => {
  ...
  it('menandai path aktif', async () => {
    render(<WarmEditorialMobileNav categories={kanal(3)} path="/tentang" />);
    fireEvent.click(screen.getByRole('button', { name: 'Informasi' }));
