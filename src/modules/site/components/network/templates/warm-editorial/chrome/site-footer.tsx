          ...
          <ul className="m-0 mt-4 list-none space-y-2.5 p-0">
            {categories.map((item) => (
              <li key={`${item.href}:${item.label}`} className="m-0 p-0">
                <Link href={item.href} className="font-sans text-sm text-slate-600 transition-colors hover:text-[#b4532a]">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <p className="m-0 mt-4 flex items-center gap-2">
            ...
            <input
              ...
              className="h-10 w-full min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3.5 font-sans text-sm text-slate-900 placeholder:text-slate-400 focus:border-[#b4532a] focus:outline-none"
            />
