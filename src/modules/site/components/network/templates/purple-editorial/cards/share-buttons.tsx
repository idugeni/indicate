 ...
 */
export function PurpleEditorialShareButtons({ article, canonical }: { readonly article: ArticleListItem; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#7c3aed]';
  ...
  return (
    <p className="m-0 flex flex-none flex-wrap items-center justify-center gap-2 sm:justify-start" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[#7c3aed] text-white transition-colors hover:bg-[#5f21d6]"
      >
      ...
      <button
        ...
        className={`${round} ${copied ? '!bg-[#7c3aed] !text-white !ring-[#7c3aed]' : ''}`}
      >
        {copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Link2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <span aria-live="polite" className="font-sans text-xs font-semibold text-[#7c3aed]">
        {copied ? 'Tersalin!' : ''}
      </span>
    </p>
  );
