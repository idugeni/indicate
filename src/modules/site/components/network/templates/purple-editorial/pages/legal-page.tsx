 ...
 */
export function PurpleEditorialLegal({ site, title, description, path = '/', sections, effectiveDate }: PurpleEditorialLegalProps) {
            ...
            {sections.map((section, index) => (
              <li key={`${index}:${section.heading}`} className="border-b border-slate-100 last:border-b-0">
                <a
                  ...
                  className="group flex items-baseline gap-3 py-2 font-sans text-sm text-slate-600 transition-colors hover:text-[#7c3aed]"
                >
                  <span className="flex-none font-mono text-[11px] tabular-nums text-[#7c3aed]">
          ...
          {sections.map((section, index) => (
            ...
            >
              <div className="flex items-baseline gap-3">
                <span
                  aria-hidden="true"
                  className="flex h-6 w-6 flex-none items-center justify-center rounded-lg bg-[#ede9fe] font-mono text-[11px] font-bold tabular-nums text-[#7c3aed]"
                >
                  {index + 1}
                </span>
