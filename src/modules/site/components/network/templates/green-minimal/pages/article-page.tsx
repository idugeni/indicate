...
import { articleImage, authorDisplayName, formatDate, formatFullViews, isLocalImageSrc, readingMinutes } from '@/modules/site/components/network/templates/green-minimal/lib/format';
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 md:py-12">
        <article>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60 sm:px-5">
            ...
            <GreenMinimalShareButtons article={article} canonical={canonical} />
          </div>
          </header>

          <aside aria-label="Catatan editorial" className="mt-6 border-l-[3px] border-[#1d7a38] pl-4">
            <p className="m-0 flex items-center gap-1.5 font-sans text-sm font-bold text-[#1d7a38]">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-2" aria-label="Topik artikel">
              {article.tags.map((tag) => (
                <Link
                  ...
                  className="rounded-full bg-white px-3 py-1.5 font-sans text-xs font-medium text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#1d7a38]"
                >
                  #{tag}
                </Link>
              ))}
          ...
          <footer className="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 sm:p-6">
            <div className="flex items-center gap-4">
              {article.publisherLogoUrl ? (
                ...
                <span
                  aria-hidden="true"
                  className="flex h-14 w-14 flex-none items-center justify-center rounded-xl bg-[#1d7a38]/10 font-sans text-xl font-bold text-[#1d7a38]"
                >
              ...
              <div className="min-w-0">
                <p className="m-0 flex min-w-0 items-center gap-1.5 truncate font-sans text-base font-bold text-slate-900">
                  ...
                  {article.publisherVerified ? (
                    <BadgeCheck className="h-4 w-4 flex-none text-[#1d7a38]" aria-label="Penerbit terverifikasi" />
                  ) : null}
                </p>
                ...
                {publisherChannels.map((channel) => {
                  ...
                  return (
                    <a
                      ...
                      title={channel.label}
                      className="flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#1d7a38]"
                    >
            ...
            <nav aria-label="Navigasi artikel" className="mt-10 grid grid-cols-1 gap-3 border-t border-slate-200 pt-6 sm:grid-cols-2 sm:gap-4">
              <div className={`min-w-0 ${newer !== null && older === null ? 'col-span-2' : ''}`}>
                {newer !== null ? (
                  <Link
                    href={`/${newer.slug}`}
                    className="group flex h-full items-start gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60 transition-all hover:shadow-md hover:ring-[#1d7a38]/50 sm:p-5"
                  >
                    <Image
                    ...
                    />
                    <span className="flex h-9 w-9 flex-none items-center justify-center rounded-full bg-[#e0f0e5] text-[#1d7a38] transition-colors group-hover:bg-[#1d7a38] group-hover:text-white">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block font-sans text-sm font-semibold leading-snug text-slate-900 group-hover:text-[#1d7a38]">
                        {newer.title}
                      </span>
                    </span>
                  </Link>
              ...
              <div className={`min-w-0 ${older !== null && newer === null ? 'col-span-2' : ''}`}>
                {older !== null ? (
                  <Link
                    href={`/${older.slug}`}
                    className="group flex h-full flex-row-reverse items-start gap-3 rounded-2xl bg-white p-4 text-right shadow-sm ring-1 ring-slate-200/60 transition-all hover:shadow-md hover:ring-[#1d7a38]/50 sm:p-5"
                  >
                    <Image
          ...
          <aside aria-label="Laporkan konten" className="mt-8 flex flex-col gap-4 rounded-2xl bg-[#1d7a38]/5 p-4 ring-1 ring-[#1d7a38]/15 sm:flex-row sm:items-center sm:p-5">
            <span className="flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-[#1d7a38]/10 text-[#1d7a38]">
            ...
            <Link
              ...
              className="inline-flex flex-none items-center justify-center rounded-xl px-4 py-2.5 font-sans text-sm font-bold text-[#1d7a38] ring-1 ring-[#1d7a38]/30 transition-colors hover:bg-[#1d7a38] hover:text-white"
            >
              Laporkan konten
            </Link>
          </aside>
