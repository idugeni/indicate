...
import { GlassyBluePrimaryButton } from '@/modules/site/components/network/templates/glassy-blue/ui/glassy-blue-button';
import { GlassyBlueInput } from '@/modules/site/components/network/templates/glassy-blue/ui/glassy-blue-input';
 ...
 */
export function GlassyBlueMobileSidebar({
  open,
  onClose,
  onFocusReturn,
  closeRef,
    ...
    <div aria-hidden={!open} inert={!open} className={cn('fixed inset-0 z-[60] lg:hidden', open ? 'pointer-events-auto' : 'pointer-events-none')}>
      ...
      >
        ...
        <form role="search" onSubmit={submitSidebar} className="flex flex-none items-center gap-2 border-b border-slate-100 px-5 py-3">
          <label htmlFor="glassy-blue-sidebar-search" className="sr-only">
            Cari berita
          </label>
          <GlassyBlueInput
            id="glassy-blue-sidebar-search"
            value={sidebarQuery}
          ...
          />
          <GlassyBluePrimaryButton type="submit" aria-label="Cari" className="h-9 w-9 flex-none rounded-full p-0">
            <Search className="h-4 w-4" aria-hidden="true" />
          </GlassyBluePrimaryButton>
        </form>
