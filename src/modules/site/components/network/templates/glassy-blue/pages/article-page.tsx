      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 md:py-12">
        <article>
          <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 font-sans text-xs text-slate-600">
            <Link href="/" className="transition-colors hover:text-[#1f7cff]">
              Beranda
            </Link>
            <ChevronRight className="h-3 w-3" aria-hidden="true" />
            {article.categorySlug ? (
              <>
                <Link href={`/categories/${article.categorySlug}`} className="transition-colors hover:text-[#1f7cff]">
          ...
          {article.categoryName === null ? null : (
            <p className="m-0 mt-6 flex items-center gap-2 font-sans text-sm font-semibold text-[#1f7cff]">
          ...
          <p className="m-0 mt-4 block w-full border-l-[3px] border-[#1f7cff] pl-4 font-sans text-[19px] font-medium leading-[1.7] text-slate-700">
          ...
          <aside aria-label="Catatan editorial" className="mt-6 border-l-[3px] border-[#1f7cff] pl-4">
            <p className="m-0 flex items-center gap-1.5 font-sans text-sm font-bold text-[#1f7cff]">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-2" aria-label="Topik artikel">
              {article.tags.map((tag) => (
                <Link
                  ...
                  className="rounded-full bg-white px-3 py-1.5 font-sans text-xs font-medium text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#1f7cff]"
                >
                  #{tag}
                </Link>
              ))}
          ...
          <footer className="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 sm:p-6">
            <div className="flex items-center gap-4">
              {article.publisherLogoUrl ? (
                ...
                <span
                  aria-hidden="true"
                  className="flex h-14 w-14 flex-none items-center justify-center rounded-xl bg-[#1f7cff]/10 font-sans text-xl font-bold text-[#1f7cff]"
                >
              ...
              <div className="min-w-0">
                <p className="m-0 flex min-w-0 items-center gap-1.5 truncate font-sans text-base font-bold text-slate-900">
                  ...
                  {article.publisherVerified ? (
                    <BadgeCheck className="h-4 w-4 flex-none text-[#1f7cff]" aria-label="Penerbit terverifikasi" />
                  ) : null}
                </p>
                ...
                {newer !== null ? (
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block font-sans text-sm font-semibold leading-snug text-slate-900 group-hover:text-[#1f7cff]">
                        {newer.title}
                      </span>
                    </span>
                  </Link>
          ...
          <aside aria-label="Laporkan konten" className="mt-8 flex flex-col gap-4 rounded-2xl bg-[#1f7cff]/5 p-4 ring-1 ring-[#1f7cff]/15 sm:flex-row sm:items-center sm:p-5">
            <span className="flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-[#1f7cff]/10 text-[#1f7cff]">
            ...
            <Link
              ...
              className="inline-flex flex-none items-center justify-center rounded-xl px-4 py-2.5 font-sans text-sm font-bold text-[#1f7cff] ring-1 ring-[#1f7cff]/30 transition-colors hover:bg-[#1f7cff] hover:text-white"
            >
              Laporkan konten
            </Link>
          </aside>
