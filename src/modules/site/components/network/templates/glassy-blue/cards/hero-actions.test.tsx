...
describe('GlassyBlueHeroActions', () => {
  it('menyimpan dan melepas simpanan lokal', () => {
    render(<GlassyBlueHeroActions slug="berita-x" title="Judul X" />);
    ...
    expect(window.localStorage.getItem('glassy-blue:bookmark:berita-x')).toBe('1');
  ...
  it('memulihkan status tersimpan dari localStorage', () => {
    window.localStorage.setItem('glassy-blue:bookmark:berita-x', '1');
    render(<GlassyBlueHeroActions slug="berita-x" title="Judul X" />);
