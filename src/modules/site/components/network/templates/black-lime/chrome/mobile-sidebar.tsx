 ...
 */
export function BlackLimeMobileSidebar({
  open,
  onClose,
  onFocusReturn,
  closeRef,
  ...
  return createPortal(
    <div aria-hidden={!open} inert={!open} className={cn('fixed inset-0 z-[60] lg:hidden', open ? 'pointer-events-auto' : 'pointer-events-none')}>
      ...
      <div
        ...
        className={cn(
          'absolute inset-y-0 right-0 flex w-[min(22rem,88vw)] flex-col bg-[#0a0c07] shadow-2xl ring-1 ring-[#242b1f] transition-transform duration-300 ease-out',
