...
import { BlackLimeHeroActions } from '@/modules/site/components/network/templates/black-lime/cards/hero-actions';
...
describe('BlackLimeHeroActions', () => {
  it('menyimpan dan melepas simpanan lokal', () => {
    render(<BlackLimeHeroActions slug="berita-x" title="Judul X" />);
    ...
    expect(window.localStorage.getItem('black-lime:bookmark:berita-x')).toBe('1');
  ...
  it('memulihkan status tersimpan dari localStorage', () => {
    window.localStorage.setItem('black-lime:bookmark:berita-x', '1');
    render(<BlackLimeHeroActions slug="berita-x" title="Judul X" />);
