  ...
  return (
    <p className="m-0 flex flex-none flex-wrap items-center gap-2" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[#c5f82a] text-white transition-colors hover:bg-[#9ecb14]"
      >
      ...
      <button
        ...
        className={`${round} ${copied ? '!bg-[#c5f82a] !text-white !ring-[#c5f82a]' : ''}`}
      >
        {copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Link2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <span aria-live="polite" className="font-sans text-xs font-semibold text-[#c5f82a]">
        {copied ? 'Tersalin!' : ''}
      </span>
    </p>
  );
