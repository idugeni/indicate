...
import { formatDate, tickerHeadline } from '@/modules/site/components/network/templates/orange-modern/lib/format';
...
export function OrangeModernTicker({ articles }: { readonly articles: readonly ArticleListItem[] }) {
  ...
  return (
    ...
    >
      <span className="flex-none self-start rounded-full bg-[#ea580c] px-3.5 py-1.5 font-sans text-xs font-bold tracking-wide text-white sm:self-auto">
            ...
            {items.map((item, position) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  position === index ? 'w-5 bg-[#ea580c]' : 'w-1.5 bg-slate-300 hover:bg-slate-400'
                }`}
              />
            ))}
          </span>
        ...
        {items.length > 1 ? (
          <span className="flex flex-none items-center gap-1.5">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full ring-1 ring-slate-200 transition-colors hover:text-[#ea580c]"
            >
