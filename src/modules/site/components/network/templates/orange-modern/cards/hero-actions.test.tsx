...
import { OrangeModernHeroActions } from '@/modules/site/components/network/templates/orange-modern/cards/hero-actions';
...
describe('OrangeModernHeroActions', () => {
  it('menyimpan dan melepas simpanan lokal', () => {
    render(<OrangeModernHeroActions slug="berita-x" title="Judul X" />);
    ...
    expect(window.localStorage.getItem('orange-modern:bookmark:berita-x')).toBe('1');
  ...
  it('memulihkan status tersimpan dari localStorage', () => {
    window.localStorage.setItem('orange-modern:bookmark:berita-x', '1');
    render(<OrangeModernHeroActions slug="berita-x" title="Judul X" />);
