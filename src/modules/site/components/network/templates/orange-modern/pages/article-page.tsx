...
import { articleImage, authorDisplayName, formatDate, formatFullViews, isLocalImageSrc, readingMinutes } from '@/modules/site/components/network/templates/orange-modern/lib/format';
        ...
        <article>
          <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 font-sans text-xs text-slate-600">
            ...
            {article.categorySlug ? (
              <>
                <Link href={`/categories/${article.categorySlug}`} className="transition-colors hover:text-[#ea580c]">
          ...
          {article.categoryName === null ? null : (
            <p className="m-0 mt-6 flex items-center gap-2 font-sans text-sm font-semibold text-[#ea580c]">
          ...
          <p className="m-0 mt-4 block w-full border-l-[3px] border-[#ea580c] pl-4 font-sans text-[19px] font-medium leading-[1.7] text-slate-700">
          ...
          <aside aria-label="Catatan editorial" className="mt-6 border-l-[3px] border-[#ea580c] pl-4">
            <p className="m-0 flex items-center gap-1.5 font-sans text-sm font-bold text-[#ea580c]">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-2" aria-label="Topik artikel">
              {article.tags.map((tag) => (
                <Link
                  ...
                  className="rounded-full bg-white px-3 py-1.5 font-sans text-xs font-medium text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#ea580c]"
                >
                  #{tag}
                </Link>
              ))}
          ...
          <footer className="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 sm:p-6">
            <div className="flex items-center gap-4">
              {article.publisherLogoUrl ? (
                ...
                <span
                  aria-hidden="true"
                  className="flex h-14 w-14 flex-none items-center justify-center rounded-xl bg-[#ea580c]/10 font-sans text-xl font-bold text-[#ea580c]"
                >
              ...
              <div className="min-w-0">
                <p className="m-0 flex min-w-0 items-center gap-1.5 truncate font-sans text-base font-bold text-slate-900">
                  ...
                  {article.publisherVerified ? (
                    <BadgeCheck className="h-4 w-4 flex-none text-[#ea580c]" aria-label="Penerbit terverifikasi" />
                  ) : null}
                </p>
            ...
            <nav aria-label="Navigasi artikel" className="mt-10 grid grid-cols-1 gap-3 border-t border-slate-200 pt-6 sm:grid-cols-2 sm:gap-4">
              <div className={`min-w-0 ${newer !== null && older === null ? 'col-span-2' : ''}`}>
                {newer !== null ? (
                  <Link
                    href={`/${newer.slug}`}
                    className="group flex h-full items-start gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/60 transition-all hover:shadow-md hover:ring-[#ea580c]/50 sm:p-5"
                  >
                    <Image
                    ...
                    />
                    <span className="flex h-9 w-9 flex-none items-center justify-center rounded-full bg-[#ffedd5] text-[#ea580c] transition-colors group-hover:bg-[#ea580c] group-hover:text-white">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block font-sans text-sm font-semibold leading-snug text-slate-900 group-hover:text-[#ea580c]">
                        {newer.title}
                      </span>
                    </span>
                  </Link>
              ...
              <div className={`min-w-0 ${older !== null && newer === null ? 'col-span-2' : ''}`}>
                {older !== null ? (
                  <Link
                    href={`/${older.slug}`}
                    className="group flex h-full flex-row-reverse items-start gap-3 rounded-2xl bg-white p-4 text-right shadow-sm ring-1 ring-slate-200/60 transition-all hover:shadow-md hover:ring-[#ea580c]/50 sm:p-5"
                  >
                    <Image
          ...
          <aside aria-label="Laporkan konten" className="mt-8 flex flex-col gap-4 rounded-2xl bg-[#ea580c]/5 p-4 ring-1 ring-[#ea580c]/15 sm:flex-row sm:items-center sm:p-5">
            <span className="flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-[#ea580c]/10 text-[#ea580c]">
            ...
            <Link
              ...
              className="inline-flex flex-none items-center justify-center rounded-xl px-4 py-2.5 font-sans text-sm font-bold text-[#ea580c] ring-1 ring-[#ea580c]/30 transition-colors hover:bg-[#ea580c] hover:text-white"
            >
              Laporkan konten
            </Link>
          </aside>
