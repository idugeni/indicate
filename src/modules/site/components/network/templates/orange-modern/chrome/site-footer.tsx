...
import { OrangeModernStoreBadges } from '@/modules/site/components/network/templates/orange-modern/chrome/store-badges';
          ...
          <ul className="m-0 mt-4 list-none space-y-2.5 p-0">
            {categories.map((item) => (
              <li key={`${item.href}:${item.label}`} className="m-0 p-0">
                <Link href={item.href} className="font-sans text-sm text-slate-600 transition-colors hover:text-[#ea580c]">
                  {item.label}
                </Link>
              </li>
            ))}
