...
import { RedEditorialStoreBadges } from '@/modules/site/components/network/templates/red-editorial/chrome/store-badges';
  ...
  return (
    <footer className="border-t border-slate-200 bg-white">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-x-6 gap-y-10 px-4 py-12 sm:grid-cols-2 sm:gap-10 sm:px-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <ul className="m-0 mt-4 list-none space-y-2.5 p-0">
            {categories.map((item) => (
              <li key={`${item.href}:${item.label}`} className="m-0 p-0">
                <Link href={item.href} className="font-sans text-sm text-slate-600 transition-colors hover:text-[#b91c1c]">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div className="relative col-span-2 overflow-hidden sm:col-span-1">
