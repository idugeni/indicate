...
export function RedEditorialHeroActions({ slug, title }: { readonly slug: string; readonly title: string }) {
  const [saved, setSaved] = useState(() => {
    try {
      return typeof window !== 'undefined' && window.localStorage.getItem(`red-editorial:bookmark:${slug}`) === '1';
    } catch {
      return false;
    }
  });
  ...
  const toggleSave = () => {
    const next = !saved;
    try {
      window.localStorage.setItem(`red-editorial:bookmark:${slug}`, next ? '1' : '0');
    } catch {
  ...
  const button =
    'flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#b91c1c] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#b91c1c]';
  ...
  return (
    <p className="m-0 flex flex-none items-center gap-2">
      <button
        ...
        className={`${button} ${saved ? '!bg-[#b91c1c] !text-white !ring-[#b91c1c]' : ''}`}
      >
      ...
      <button
        ...
        className={`${button} ${copied ? '!bg-[#b91c1c] !text-white !ring-[#b91c1c]' : ''}`}
      >
