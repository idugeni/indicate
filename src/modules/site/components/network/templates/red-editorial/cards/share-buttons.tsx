 ...
 */
export function RedEditorialShareButtons({ article, canonical }: { readonly article: NetworkArticle; readonly canonical: string }) {
  ...
  const round =
    'flex h-9 w-9 items-center justify-center rounded-full text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#b91c1c]';
  ...
  return (
    <p className="m-0 flex flex-none flex-wrap items-center gap-2" aria-label="Bagikan artikel">
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full bg-[#b91c1c] text-white transition-colors hover:bg-[#7f1212]"
      >
      ...
      <button
        ...
        className={`${round} ${copied ? '!bg-[#b91c1c] !text-white !ring-[#b91c1c]' : ''}`}
      >
        {copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Link2 className="h-4 w-4" aria-hidden="true" />}
      </button>
      <span aria-live="polite" className="font-sans text-xs font-semibold text-[#b91c1c]">
        {copied ? 'Tersalin!' : ''}
      </span>
    </p>
  );
