...
describe('RedEditorialViewBeacon', () => {
  it('mengirim beacon sekali saat mount', () => {
    const send = installBeacon(true);
    const { container } = render(<RedEditorialViewBeacon organizationId={O} siteId={S} articleSiteId={A} />);
  ...
  it('memakai fetch saat beacon ditolak', async () => {
    ...
    try {
      render(<RedEditorialViewBeacon organizationId={O} siteId={S} articleSiteId={A} />);
  ...
  it('diam untuk id tidak valid', () => {
    const send = installBeacon(true);
    render(<RedEditorialViewBeacon organizationId="bukan-uuid" siteId={S} articleSiteId={A} />);
