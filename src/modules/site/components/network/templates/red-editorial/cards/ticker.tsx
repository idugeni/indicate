...
export function RedEditorialTicker({ articles }: { readonly articles: readonly ArticleListItem[] }) {
  ...
  return (
    ...
    >
      ...
      <span className="flex-none self-start rounded-full bg-[var(--tpl-primary,#b91c1c)] px-3.5 py-1.5 font-sans text-xs font-bold tracking-wide text-white sm:self-auto">
      ...
      <p
        ...
        aria-live="polite"
      >
        <Link href={`/${article.slug}`} className="no-underline hover:text-[var(--tpl-primary,#b91c1c)]">
            ...
            {items.map((item, position) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  position === index ? 'w-5 bg-[var(--tpl-primary,#b91c1c)]' : 'w-1.5 bg-slate-300 hover:bg-slate-400'
                }`}
              />
            ))}
          </span>
        ...
        {items.length > 1 ? (
          <span className="flex flex-none items-center gap-1.5">
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full ring-1 ring-slate-200 transition-colors hover:text-[var(--tpl-primary,#b91c1c)]"
            >
