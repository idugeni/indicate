...
export function RedEditorialSearchForm({ query }: { readonly query: string }) {
  return (
    <section aria-label="Pencarian berita" className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 sm:p-6">
      ...
      <Form className="mt-4" action="/search" role="search">
        <label htmlFor="red-editorial-search" className="sr-only">
          Cari berita
        </label>
        <div className="flex flex-col gap-2.5 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input
              id="red-editorial-search"
              name="q"
              defaultValue={query}
              maxLength={120}
              autoComplete="off"
              placeholder="Ketik kata kunci…"
              className="h-11 w-full appearance-none rounded-full border border-slate-200 bg-[#fffafa] pl-11 pr-4 font-sans text-base text-slate-900 placeholder:text-slate-400 focus:border-[#b91c1c] focus:outline-none sm:text-sm"
            />
          </div>
          <button
            type="submit"
            className="inline-flex h-11 flex-none items-center justify-center rounded-full bg-[#b91c1c] px-6 font-sans text-sm font-bold text-white transition-colors hover:bg-[#7f1212]"
          >
            Cari
          </button>
        </div>
...
export function RedEditorialSearchResults({ articles, query }: { readonly articles: readonly ArticleListItem[]; readonly query: string }) {
