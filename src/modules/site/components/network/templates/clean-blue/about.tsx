      ...
      <CleanBlueContainer className="space-y-6 py-6 md:py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60">
            ...
            <p className="m-0 mt-1 font-sans text-sm text-slate-500">Artikel terbit di kanal ini</p>
          </div>
          <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60">
            <p className="m-0 font-mono text-2xl font-bold tabular-nums text-slate-900">{categories.length}</p>
            <p className="m-0 mt-1 font-sans text-sm text-slate-500">Kanal liputan aktif</p>
          </div>
        </div>
              ...
              {categories.map(([slug, name]) => (
                <li key={slug} className="m-0">
                  <Link
                    ...
                    className="inline-block rounded-full bg-white px-4 py-2 font-sans text-sm font-semibold text-[#1f6feb] shadow-sm ring-1 ring-slate-200/60 transition-colors hover:bg-[#1f6feb] hover:text-white"
                  >
                    {name}
                  </Link>
                </li>
