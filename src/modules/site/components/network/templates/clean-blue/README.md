...
- `ui/` — primitif: container, status-line, empty, author-avatar,
  article-meta, section-heading, clean-blue-input, clean-blue-button, loader.
- `seo/` — JSON-LD mandiri.
...
2. Template ini terang permanen di bawah root `<html class="dark">` — form
   wajib pakai `ui/clean-blue-input.tsx` / `ui/clean-blue-button.tsx`,
   jangan `Input`/`Button` shadcn langsung.
3. Tambah halaman baru di `pages/`, daftarkan ke registry `CLEAN_BLUE_PAGES`.
