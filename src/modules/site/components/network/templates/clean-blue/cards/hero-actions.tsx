...
export function CleanBlueHeroActions({ slug, title }: { readonly slug: string; readonly title: string }) {
  const [saved, setSaved] = useState(() => {
    try {
      return typeof window !== 'undefined' && window.localStorage.getItem(`clean-blue:bookmark:${slug}`) === '1';
    } catch {
      return false;
    }
  });
  ...
  const toggleSave = () => {
    const next = !saved;
    try {
      window.localStorage.setItem(`clean-blue:bookmark:${slug}`, next ? '1' : '0');
    } catch {
  ...
  const button =
    'flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-600 ring-1 ring-slate-200 transition-colors hover:text-[#1a5fd0] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#1a5fd0]';
  ...
  return (
    <p className="m-0 flex flex-none items-center gap-2">
      <button
        ...
        className={`${button} ${saved ? '!bg-[#1a5fd0] !text-white !ring-[#1a5fd0]' : ''}`}
      >
      ...
      <button
        ...
        className={`${button} ${copied ? '!bg-[#1a5fd0] !text-white !ring-[#1a5fd0]' : ''}`}
      >
