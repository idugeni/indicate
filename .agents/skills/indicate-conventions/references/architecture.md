...
| R2 buckets (private + public) | Media objects via S3-compatible API; `article-cover` under `pub/` routes to the public bucket served direct via `media.indicate.website`, everything else stays private behind signed routes | No list grants or per-tenant buckets (a separate optional audit/WORM bucket may exist for exports); one app credential must cover both media buckets |
