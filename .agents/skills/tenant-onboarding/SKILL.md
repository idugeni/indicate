...
- Key R2: tenant-scoped `o/{org}/p/site-{logo|favicon}/.../{siteId}/...` (sanitasi: lowercase, non-alnum → `-`; token acak aman minimal 12 karakter). Bucket privat (nama dari konfigurasi deployment); tidak ada folder per domain. Artikel (`article-cover`/`article-image`) memakai prefix `pub/` di bucket publik — logo/favicon TIDAK PERNAH publik.
