...
const actor = { actorType: 'user' as const, actorId: 'user', verifiedAuthUserId: 'auth', organizationId: '00000000-0000-4000-8000-000000000001', permissionSet: new Set(['sites.manage']), entryPoint: 'cms' as const, requestId: 'request' };
...
describe('Stage 5 durable recovery boundaries', () => {
  it('resumes from the persisted phase without replaying completed provider phases', async () => {
    const repo = createStage5E2eRepository(); const cloudflare = new CloudflareFake(); const vercel = new VercelFake();
    await repo.deactivateSite(actor, '00000000-0000-4000-8000-000000000103', 'alpha.example.web.id', planInvalidation({ kind: 'hostname', organizationId: actor.organizationId, siteId: '00000000-0000-4000-8000-000000000103', previousHostname: 'alpha.example.web.id', currentHostname: null }), '2026-01-01T00:00:00.000Z');
    let attempt = await repo.beginActivation(actor, '00000000-0000-4000-8000-000000000103', 'alpha.example.web.id', 'alpha.example.web.id', '2026-01-01T00:00:01.000Z');
  ...
  it('keeps failed Vercel deactivation cleanup durable and independently retryable', async () => {
    const repo = createStage5E2eRepository(); const vercel = new VercelFake(); vercel.failRemoval = true;
    const service = new DomainProvisioningService(repo, new CloudflareFake(), vercel, { verifyPendingHostname: async () => true }, new Set(), [1], 3);
    await expect(service.deactivate(actor, '00000000-0000-4000-8000-000000000103', 'alpha.example.web.id', new Date('2026-01-01T00:00:00.000Z'))).rejects.toThrow(Stage5OperationPendingError);
  ...
  it('records database retry progress and bypass even when Redis coordination also fails', async () => {
    const repo = createStage5E2eRepository(); const plan = planInvalidation({ kind: 'site_settings', organizationId: actor.organizationId, siteId: '00000000-0000-4000-8000-000000000103', hostname: 'alpha.example.web.id' });
    ...
    expect(repo.snapshot().bypasses).toContain(`${actor.organizationId}:00000000-0000-4000-8000-000000000103`);
  });
});
