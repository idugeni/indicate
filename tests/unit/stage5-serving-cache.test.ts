...
describe('Stage 5 serving-path cache safety', () => {
  it('discards a cached entry whose embedded Site context differs from current exact-host discovery', async () => {
    const repository = createStage5E2eRepository(); const context = (await repository.findActiveSitesByExactHostname('alpha.example.web.id'))[0]!; const expected = await repository.loadPublicSite(context, {});
  ...
  it('consults durable Site bypass before any cache read', async () => {
    const repository = createStage5E2eRepository(); const context = (await repository.findActiveSitesByExactHostname('alpha.example.web.id'))[0]!;
