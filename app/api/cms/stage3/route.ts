...
import { getStage3E2eFixture } from '@/app/cms/stage3-composition';
...
const querySchema = z.object({
  ...
  publicationState: z.enum(['queued', 'processing', 'published', 'failed', 'retrying']).optional(), search: z.string().max(300).optional(),
...
async function contextFor(organizationId: string, requestId: string): Promise<ContextResult> {
  ...
  if (isE2eMode()) {
    ...
    return { actor: createStage3Actor(organizationId, requestId), service: getStage3E2eFixture().service, close: async () => undefined };
  }
  ...
  if (membership === null || !membership.roleActive) {
    const deniedActor: AuthorizedTenantActorContext = { actorType: 'user', actorId: local.value.id, verifiedAuthUserId: identity.authUserId, organizationId, permissionSet: new Set(), entryPoint: 'cms', requestId };
    try { await new DrizzleStage3Repository(runtime.db).recordDenied(deniedActor, 'cms.organization.authorize', 'organization'); }
  ...
  return {
    actor: { actorType: 'user', actorId: local.value.id, verifiedAuthUserId: identity.authUserId, organizationId, permissionSet: new Set(membership.permissions), entryPoint: 'cms', requestId },
    service: new TenantBusinessService(new DrizzleStage3Repository(runtime.db), new UuidGenerator()), close: runtime.close,
  };
}
...
export async function GET(request: Request) {
  ...
  const context = await contextFor(parsed.data.organizationId, requestId); if (isContextError(context)) return NextResponse.json(context, { status: responseStatus(context) });
  try {
