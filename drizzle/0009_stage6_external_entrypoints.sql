-- Stage 6 external entry points: credential metadata, durable Telegram conversations,
...
CREATE OR REPLACE FUNCTION indicate_private.stage6_ensure_org_permissions(p_organization_id uuid)
RETURNS void
...
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_ensure_org_permissions(uuid) FROM PUBLIC;--> statement-breakpoint
SELECT indicate_private.stage6_ensure_org_permissions(id) FROM public.organizations;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.resolve_stage6_api_key_lookup(p_lookup_id text)
RETURNS TABLE (
...
AS $$
  ...
  LIMIT 1
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.resolve_stage6_api_key_lookup(text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.resolve_stage6_api_key_lookup(text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.resolve_stage6_telegram_identity(p_user_id text, p_chat_id text)
RETURNS TABLE (
...
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.resolve_stage6_telegram_identity(text, text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.resolve_stage6_telegram_identity(text, text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_replay_claim_organization(p_source text, p_replay_id text)
RETURNS uuid
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, indicate_private
AS $$
...
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_replay_claim_organization(text, text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_replay_claim_organization(text, text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_claim_replay(
...
AS $$
  ...
  LIMIT 1
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_claim_replay(text, text, uuid, text, timestamptz, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_claim_replay(text, text, uuid, text, timestamptz, timestamptz) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_finish_replay(
...
AS $$
  ...
  LIMIT 1
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_finish_replay(text, text, text, public.replay_claim_status, jsonb, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_finish_replay(text, text, text, public.replay_claim_status, jsonb, timestamptz) TO indicate_runtime;--> statement-breakpoint
...
CREATE OR REPLACE FUNCTION indicate_private.stage6_has_tenant_permission(p_actor_id uuid, p_organization_id uuid, p_permission text)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, indicate_private
AS $$
...
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_has_tenant_permission(uuid, uuid, text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_has_tenant_permission(uuid, uuid, text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_list_customers(p_actor_id uuid)
RETURNS TABLE (
...
AS $$
BEGIN
  IF NOT indicate_private.stage6_has_platform_permission(p_actor_id, 'platform.customer.admin') THEN
...
END
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_list_customers(uuid) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_list_customers(uuid) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_create_customer(
...
BEGIN
  ...
  PERFORM indicate_private.stage6_ensure_org_permissions(p_organization_id);
...
END
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_create_customer(uuid, text, uuid, text, text, jsonb, jsonb, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_create_customer(uuid, text, uuid, text, text, jsonb, jsonb, timestamptz) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_update_customer(
...
END
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_update_customer(uuid, text, uuid, integer, text, text, public.record_status, jsonb, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_update_customer(uuid, text, uuid, integer, text, text, public.record_status, jsonb, timestamptz) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.stage6_update_subscription(
  p_actor_id uuid, p_request_id text, p_organization_id uuid, p_expected_version integer,
...
BEGIN
  v_platform := indicate_private.stage6_has_platform_permission(p_actor_id, 'platform.customer.admin');
  IF NOT v_platform AND (
    indicate_private.current_organization_id() IS DISTINCT FROM p_organization_id
    OR NOT indicate_private.stage6_has_tenant_permission(p_actor_id, p_organization_id, 'subscription.manage')
  ) THEN
...
END
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.stage6_update_subscription(uuid, text, uuid, integer, text, public.subscription_status, timestamptz, timestamptz, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.stage6_update_subscription(uuid, text, uuid, integer, text, public.subscription_status, timestamptz, timestamptz, timestamptz) TO indicate_runtime;--> statement-breakpoint

INSERT INTO public.indicate_schema_migrations(version, name, checksum)
VALUES (10, 'stage6_external_entrypoints', 'stage6-external-entrypoints-v1');
