-- Stage 5 production-boundary hardening: RLS-safe discovery, durable recovery, and fenced invalidation.
...
CREATE OR REPLACE FUNCTION indicate_private.resolve_stage5_public_host(p_hostname text)
RETURNS TABLE (
...
REVOKE ALL ON FUNCTION indicate_private.resolve_stage5_public_host(text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.resolve_stage5_public_host(text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.is_stage5_pending_host(p_hostname text, p_attempt_id uuid)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, indicate_private
AS $$
  SELECT EXISTS (
    ...
    WHERE a.id = p_attempt_id
      ...
      AND a.phase IN ('pending', 'cloudflare_verified', 'vercel_associated')
      AND s.normalized_hostname = p_hostname
      AND s.status = 'inactive'
      AND s.activation_state = 'pending'
  )
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.is_stage5_pending_host(text, uuid) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.is_stage5_pending_host(text, uuid) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.is_stage5_previous_host_owned(
...
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.is_stage5_previous_host_owned(uuid, uuid, text) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.is_stage5_previous_host_owned(uuid, uuid, text) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.claim_stage5_activation_attempts(
  p_now timestamptz,
  p_limit integer,
  p_claim_token uuid,
...
END;
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.claim_stage5_activation_attempts(timestamptz, integer, uuid, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.claim_stage5_activation_attempts(timestamptz, integer, uuid, timestamptz) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.enable_stage5_cache_bypass_on_enqueue()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, indicate_private
AS $$
...
END;
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.enable_stage5_cache_bypass_on_enqueue() FROM PUBLIC;--> statement-breakpoint
CREATE TRIGGER invalidation_tasks_enable_cache_bypass
AFTER INSERT ON public.invalidation_tasks
FOR EACH ROW EXECUTE FUNCTION indicate_private.enable_stage5_cache_bypass_on_enqueue();--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.complete_stage5_invalidation(
  p_organization_id uuid,
  p_task_id uuid,
  p_claim_token uuid,
  p_now timestamptz
...
END;
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.complete_stage5_invalidation(uuid, uuid, uuid, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.complete_stage5_invalidation(uuid, uuid, uuid, timestamptz) TO indicate_runtime;--> statement-breakpoint

CREATE OR REPLACE FUNCTION indicate_private.fail_stage5_invalidation(
  p_organization_id uuid,
  p_task_id uuid,
  p_claim_token uuid,
  p_failure jsonb,
...
END;
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION indicate_private.fail_stage5_invalidation(uuid, uuid, uuid, jsonb, timestamptz, boolean, timestamptz) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.fail_stage5_invalidation(uuid, uuid, uuid, jsonb, timestamptz, boolean, timestamptz) TO indicate_runtime;--> statement-breakpoint

INSERT INTO public.indicate_schema_migrations(version, name, checksum)
VALUES (9, 'stage5_production_boundaries', 'stage5-production-boundaries-v1');
