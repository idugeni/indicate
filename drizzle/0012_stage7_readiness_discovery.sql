-- Stage 7 production-readiness boundary: bounded exact-host discovery for the
-- NOBYPASSRLS runtime role without granting direct cross-tenant table access.
CREATE OR REPLACE FUNCTION indicate_private.discover_stage7_active_hosts(p_hostnames text[])
RETURNS TABLE (
  hostname text,
...
REVOKE ALL ON FUNCTION indicate_private.discover_stage7_active_hosts(text[]) FROM PUBLIC;--> statement-breakpoint
GRANT EXECUTE ON FUNCTION indicate_private.discover_stage7_active_hosts(text[]) TO indicate_runtime;--> statement-breakpoint

INSERT INTO public.indicate_schema_migrations(version, name, checksum)
VALUES (13, 'stage7_readiness_discovery', 'stage7-readiness-discovery-v1');
